...
//DHL - Skillet - map_background command hook
FnCommandCallback_t fnOldMapCallback;
ConCommand* map_background = NULL;
bool bShouldSetBackground = false;
void CC_MapBackgroundHook( const CCommand &command )
{
	...
	if ( !engine->IsMapValid( command.Arg(1) ) )
		return;

	//Send the command to the engine
	map_background->SetCommandCallback( fnOldMapCallback );
	map_background->Dispatch( command );

	//Back to this callback
	map_background->SetCommandCallback(FnCommandCallback_t(CC_MapBackgroundHook));

	//Take note that we are running the map as a menu background
	//DHL - Skillet - HACK!
	bShouldSetBackground = true;
}
		...
		CGlobalVars *pGlobals)
{
	...
	//DHL - Skillet - Hook the map_background ConCommand so we can figure out from the server if this is truly
	//loaded as a background map
	map_background = g_pCVar->FindCommand("map_background");
	fnOldMapCallback = map_background->GetCommandCallback();
	map_background->SetCommandCallback(FnCommandCallback_t(CC_MapBackgroundHook));
...
bool CServerGameDLL::LevelInit( const char *pMapName, char const *pMapEntities, char const *pOldLevel, char const *pLandmarkName, bool loadGame, bool background )
{
	...
	//DHL - Skillet - Hack here
	if ( bShouldSetBackground )
	{
		DHLShared::SetBackgroundMap( true );
		bShouldSetBackground = false;
	}
	...
	CBaseEntity *EntDHLMapRules = gEntList.FindEntityByClassname( NULL, "dhl_maprules" );
	//Cast our CBaseEntity into CDHLMapRules
	CDHLMapRules *DHLMapRules = static_cast<CDHLMapRules*>( EntDHLMapRules );
	if ( DHLMapRules )
	{
		DHLRules()->SetGrayscale( DHLMapRules->GetForceGrayscale() );
	}
	else
	{
		DHLRules()->SetGrayscale( false );
	}
	return true;
}
...
void CServerGameDLL::LevelShutdown( void )
{
	...
	DHLShared::SetBackgroundMap( false );
