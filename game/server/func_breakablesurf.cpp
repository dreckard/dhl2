...
void CBreakableSurface::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if (info.GetDamageType() & (DMG_BULLET | DMG_CLUB | DMG_SLASH)) //DHL - Skillet - Added DMG_SLASH
	{
...
//DHL - Skillet - Simplified function to determine if any part of the surface is broken
bool CBreakableSurface::IsBroken( void )
{
	return m_bIsBroken;
}
...
void CBreakableSurface::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	if ( !m_bIsBroken )
	{
		...
		//DHL - Skillet - Hack - Always break for grenades
		if ( FClassnameIs( pEvent->pEntities[!index], "npc_grenade_frag" ) )
			damage = 11.0f;
