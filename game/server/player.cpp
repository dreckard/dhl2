...
#include "dhl/dhl_player_inc.h" //HACK!
...
//DHL - Skillet - Modified and made FCVAR_CHEAT. NOTE: These are scripted per-weapon now
ConVar	sk_player_head( "sk_player_head","1.0", FCVAR_CHEAT );
ConVar	sk_player_chest( "sk_player_chest","1.0", FCVAR_CHEAT );
ConVar	sk_player_stomach( "sk_player_stomach","1.0", FCVAR_CHEAT );
ConVar	sk_player_arm( "sk_player_arm","1.0", FCVAR_CHEAT );
ConVar	sk_player_leg( "sk_player_leg","1.0", FCVAR_CHEAT );
...
int CBasePlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
		...
		( !attacker->IsSolidFlagSet(FSOLID_TRIGGER)) )
	{
		...
		//DHL - Was 250, we want only very small damage kicks
		if ( force.z > 50.0f )
		{
			force.z = 50.0f;
		}
		
		//DHL - Skillet - Disable them entirely for bullets & bleeding
		if ( !(GetFlags() & FL_PRONE) && !(info.GetDamageType() & DMG_BULLET) && !(info.GetDamageType() & DMG_BLEED) )
...
bool CBasePlayer::SetObserverMode(int mode )
{
	...
	if ( mode > OBS_MODE_FIXED && GetTeamNumber() > TEAM_SPECTATOR )
	{
		switch ( mp_forcecamera.GetInt() )
		{
			...
			case OBS_ALLOW_TEAM :	if ( mode != OBS_MODE_IN_EYE && mode != OBS_MODE_CHASE ) mode = OBS_MODE_IN_EYE; break; //DHL - Skillet
...
bool CBasePlayer::SetObserverTarget(CBaseEntity *target)
{
	...
	if ( m_iObserverMode == OBS_MODE_ROAMING )
	{
		...
		if ( target != this )
			JumptoPosition( target->EyePosition(), target->EyeAngles() );
	}
...
{
	//DHL - Skillet - Commented to allow spawning weapons that we have
...
bool CBasePlayer::ClientCommand( const CCommand &args )
{
	...
	else if ( stricmp( cmd, "spec_mode" ) == 0 ) // new observer mode
	{
		...
		if ( args.ArgC() >= 2 )
		{
			...
			//DHL - Skillet - Must have a valid observer target to do either of these
			if ( mode == OBS_MODE_IN_EYE || mode == OBS_MODE_CHASE )
				if ( !FindNextObserverTarget( false ) )
					return false;
		}
		else
		{
			...
			//DHL - Skillet - Must have a valid observer target to do either of these
			while ( mode == OBS_MODE_IN_EYE || mode == OBS_MODE_CHASE )
			{
				if ( !FindNextObserverTarget( false ) )
					mode++;
				else
...
void CBasePlayer::SetupVPhysicsShadow( CPhysCollide *pStandModel, const char *pStandHullName, CPhysCollide *pCrouchModel, const char *pCrouchHullName, 
									  CPhysCollide *pProneModel, const char *pProneHullName ) //DHL
{
	solid_t solid;
	...
	UpdatePhysicsShadowToPosition( GetAbsOrigin() );
...
void CBasePlayer::VPhysicsDestroyObject()
{
	...
	if ( m_pShadowProne )
	{
		m_pShadowProne->EnableCollisions( false );
...
void CBasePlayer::SetVCollisionState( const Vector &vecAbsOrigin, const Vector &vecAbsVelocity, int collisionState )
{
	...
	case VPHYS_WALK:
		...
		m_pShadowProne->EnableCollisions( false ); //DHL
	...
	case VPHYS_NOCLIP:
		...
		//DHL
	case VPHYS_PRONE:
		m_pShadowProne->SetPosition( vecAbsOrigin, vec3_angle, true );
		m_pShadowProne->SetVelocity( &vecAbsVelocity, NULL );
		m_pShadowStand->EnableCollisions( false );
		m_pShadowCrouch->EnableCollisions( false );
