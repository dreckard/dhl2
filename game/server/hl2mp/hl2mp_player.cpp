...
//Hack for linux compile. g++ won't do offsetof for SENDINFO_VECTORELEM otherwise - copied from datamap.h
#if defined(_LINUX)
...
//DHL - Skillet - We use CDHL_Player
#ifndef DISTRACTION
...
//DHL - Skillet
//#define g_ppszRandomCitizenModels DHLRules()->MobModels
//#define g_ppszRandomCombineModels DHLRules()->ProModels

//const char *g_ppszRandomCitizenModels[] = 
//{
//	//DHL - Skillet
//	"models/player/Yakuza/Yakuza.mdl",
...
const char* ppszDHLExcludedSubStrs[] =
{
	"_bodygib.mdl",
	"_headgib.mdl",
};
...
/*	if ( GetNextTeamChangeTime() >= gpGlobals->curtime )
	...
	//DHL - Skillet - do after suicide so ragdoll doesn't randomly change to the new team model
	/*if ( HL2MPRules()->IsTeamplay() == true )
	{
		SetPlayerTeamModel();
	}
...
{
	//DHL - Skillet - These assertions all get hit on bots and suck the life from the game
	if ( IsBot() )
		return;
	...
	// Must be dead.
	Assert( m_lifeState == LIFE_DEAD || m_lifeState == LIFE_RESPAWNABLE ); //DHL - Skillet - May also be respawnable
	Assert( pl.deadflag );
}
