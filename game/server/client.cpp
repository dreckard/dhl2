					...
					if ( !FindPassableSpace( pPlayer, up, 1, oldorigin ) )	// up
					{
						if ( !FindPassableSpace( pPlayer, up, -1, oldorigin ) )	// down
						{
							if ( !FindPassableSpace( pPlayer, forward, -1, oldorigin ) )	// back
							{
								...
								//DHL - Kill them if cheats are turned off and they're coming out of noclip stuck (will be forced)
								if ( !sv_cheats->GetBool() )
									ClientKill( pPlayer->edict(), vec3_origin );
							}
						}
					}
				}
...
//DHL - Removed noclip cheat flag, sv_cheats checking is done inside the callback.
