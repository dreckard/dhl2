...
void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		if ( m_Attachments.Count() > 0 )
		{
			...
			// Make an elight
			dlight_t *el = effects->CL_AllocDlight( LIGHT_INDEX_MUZZLEFLASH + index ); // DHL AllocElight -> AllocDlight
			el->origin = vAttachment;
			el->radius = random->RandomInt( 32, 64 ); 
			el->decay = el->radius / 0.05f;
			//DHL - Skillet - Scale lifetime for slow motion
			el->die = gpGlobals->curtime + 0.05f / DHLRules()->GetTimescale();
			el->color.r = 255;
			el->color.g = 192;
			el->color.b = 64;
			el->color.exponent = 5;
...
void C_BaseAnimating::DoAnimationEvents( CStudioHdr *pStudioHdr )
{
	...
	bool bDHLSkipEvents = false;
	//DHL - Skillet - Don't do view model animation events if we're in third person.
	...
	if ( ShouldMuzzleFlash() )
	{
		...
		//DHL - Skillet
		if ( !IsViewModel() && GetMoveParent() != C_BasePlayer::GetLocalPlayer() ) //For remote players...
		{
			int iAttachment = LookupAttachment( "muzzle" );
			if ( iAttachment != -1 )
				tempents->MuzzleFlash( MUZZLEFLASH_SMG1, GetRefEHandle(), iAttachment, false );
			//FireEvent( GetAbsOrigin(), GetAbsAngles(), AE_MUZZLEFLASH, "SMG1" );
		}
	}
	...
	if (bLooped)
	{
		for (int i = 0; i < (int)seqdesc.numevents; i++)
		{
			...
			//DHL - Skillet - Weird things happen if I return from the function early on before the timing code runs
			//i.e. if you fire in third person then switch to 1st some time later the VM events will be played then
			if ( !bDHLSkipEvents )
	...
	for (int i = 0; i < (int)seqdesc.numevents; i++)
	{
		...
		if ( (pevent[i].cycle > m_flPrevEventCycle && pevent[i].cycle <= flEventCycle) )
		{
			...
			//DHL - Skillet - Weird things happen if I return from the function early on before the timing code runs
			//i.e. if you fire in third person then switch to 1st some time later the VM events will be played then
			if ( !bDHLSkipEvents || pevent[ i ].event == ( AE_MUZZLEFLASH ) )
				FireEvent( GetAbsOrigin(), GetAbsAngles(), pevent[ i ].event, pevent[ i ].pszOptions() );
			else if ( pevent[ i ].event == ( CL_EVENT_EJECTBRASS1 ) )
			{
				if ( IsViewModel() )
				{
					CBasePlayer* pPlayer = ToBasePlayer(static_cast<C_BaseViewModel*>(this)->GetOwner());
					if ( pPlayer )
					{
						...
						if ( pWeapon )
						{
							Vector attachOrigin;
							QAngle attachAngles;
							if ( pWeapon->GetAttachment( 2, attachOrigin, attachAngles ) )
							{
								attachAngles.y -= 90.0f; //HACK due to misaligned attachments
								tempents->EjectBrass( attachOrigin, attachAngles, GetAbsAngles(), atoi( pevent[ i ].pszOptions() ) );
							}
						}
					}
				}
...
float C_BaseAnimating::GetSequenceCycleRate( CStudioHdr *pStudioHdr, int iSequence )
{
	...
	return Studio_CPS( pStudioHdr, pStudioHdr->pSeqdesc(iSequence), iSequence, m_flPoseParameter ) * DHLRules()->GetTimescale(); //DHL - Skillet - Slow animations
}
...
float C_BaseAnimating::SequenceDuration( CStudioHdr *pStudioHdr, int iSequence )
{
	...
	return Studio_Duration( pStudioHdr, iSequence, m_flPoseParameter ) / DHLRules()->GetTimescale(); //DHL - Skillet - Scale for time
