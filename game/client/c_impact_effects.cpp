...
//DHL - Skillet - New overload for blood effect
void FX_DustImpact( const Vector &origin, trace_t *tr, float flScale, Vector& color )
{
	//
	// PC version
	//
	...
	for ( i = 0; i < 4; i++ )
	{
		...
		if ( pParticle != NULL )
		{
			...
			// scaled
			pParticle->m_uchStartSize	= 1 * (i+1);//( unsigned char )( flScale * random->RandomInt( 3, 4 ) * (i+1) );

			// scaled
			pParticle->m_uchEndSize		= pParticle->m_uchStartSize * 2;//( unsigned char )( pParticle->m_uchStartSize * 4 );
			
			pParticle->m_uchStartAlpha	= random->RandomInt( 128, 255 );
			pParticle->m_uchEndAlpha	= 100;
	...
	for ( i = 0; i < 4; i++ )
	{
		...
		if ( pParticle != NULL )
		{
			...
			pParticle->m_uchStartSize	= flScale * random->RandomInt( 2, 4 ) * (i+1);
			pParticle->m_uchEndSize		= pParticle->m_uchStartSize * 2;
