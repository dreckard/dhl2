...
IMPLEMENT_CLIENTCLASS_DT_NOBASE(C_PlayerResource, DT_PlayerResource, CPlayerResource)
	...
	RecvPropArray3( RECVINFO_ARRAY(m_iWins), RecvPropInt( RECVINFO(m_iWins[0]))), //DHL - Skillet
END_RECV_TABLE()
...
BEGIN_PREDICTION_DATA( C_PlayerResource )
	...
	DEFINE_PRED_ARRAY( m_iWins, FIELD_INTEGER, MAX_PLAYERS+1, FTYPEDESC_PRIVATE ), //DHL - Skillet
...
//	memset( m_iPacketloss, 0, sizeof( m_iPacketloss ) );
	...
	memset( m_iWins, 0, sizeof( m_iWins ) ); //DHL - Skillet
	...
	m_Colors[TEAM_UNASSIGNED] = Color( 160, 160, 160, 255 ); //DHL: Changed to grey //COLOR_YELLOW;
#endif
...
//DHL - Skillet - LMS only
int	C_PlayerResource::GetPlayerWins( int iIndex )
{
	if ( !IsConnected( iIndex ) || !(!DHLRules()->IsTeamplay() && DHLRules()->IsRoundplay()) )
		return 0;

	return m_iWins[iIndex];
}
