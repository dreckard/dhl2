...
// DHL - Payback
//----------------------------------------------------------------------------- 
void CHud::DrawScaledIconProgressBar( int x, int y, int finalWidth, int finalHeight, CHudTexture *iconFull, CHudTexture *iconEmpty, float percentage, Color& clr, int type )
{
	// icon = full
	// icon2 = empty
	if ( iconFull == NULL || iconEmpty == NULL)
		return;
	...
	int	iconHeight = iconFull->Height();
	int	iconWidth  = iconFull->Width();

	//Draw a vertical progress bar
	if ( type == HUDPB_VERTICAL )
	{
		int	iconOfs = iconHeight * percentage;
		int scaleOfs = finalHeight * percentage;

		// Draw all of it
		iconEmpty->DrawSelfCropped(
			x, y, // Screen top left
			0, 0, // Icon top left 
			iconWidth, iconOfs, // Icon bottom right
			finalWidth, scaleOfs, // Screen bottom right 
			clr);

		// Crop it (Works!
		iconFull->DrawSelfCropped( 
			x, y + scaleOfs, // Screen top left
			0, iconOfs, // Icon top left
			iconWidth, iconHeight - iconOfs, // Icon bottom right
			finalWidth, finalHeight - scaleOfs, // Screen bottom right
			clr );
	}
	else if ( type == HUDPB_HORIZONTAL )
	{
		int	barOfs = iconWidth * percentage;

		iconEmpty->DrawSelfCropped( 
			x, y,  // Pos
			0, 0, barOfs, iconHeight, // Cropped subrect
			clr );

		iconFull->DrawSelfCropped( 
			x + barOfs, y, 
			barOfs, 0, iconWidth - barOfs, iconHeight, // Cropped subrect
			clr );
	}
}
