...
void CTextWindow::OnCommand( const char *command)
{
    if (!Q_strcmp(command, "okay"))
    {
		...
		//DHL - Skillet - Automatically show first selection panel on MOTD close
		if ( !Q_stricmp( m_szMessage, "motd" ) )
		{
			if ( DHLRules() )
			{
				IViewPortPanel* pNextPanel = NULL;
				//if ( DHLRules()->IsTeamplay() )
				{
					pNextPanel = m_pViewPort->FindPanelByName( PANEL_TEAMSELECTION );
					KeyValues *dhlData = new KeyValues("data");
					dhlData->SetString( "mapLoad", "1" );
					pNextPanel->SetData( dhlData );
					dhlData->deleteThis();
				}
				//else
				//	pNextPanel = m_pViewPort->FindPanelByName( PANEL_WEAPONSELECTION );
				if ( pNextPanel )
					m_pViewPort->ShowPanel( pNextPanel, true );
				else
					Warning( "DHL: Selection panel missing!\n" );
			}
		}
	}
