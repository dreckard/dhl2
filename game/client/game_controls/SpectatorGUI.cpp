...
void CSpectatorMenu::OnTextChanged(KeyValues *data)
{
	...
	else if ( box == m_pPlayerList )
	{
		...
		if ( kv && GameResources() )
		{
			...
			const char *currentPlayerName = currentPlayerNum > 0 ? GameResources()->GetPlayerName( currentPlayerNum ) : NULL; //DHL - Skillet

			//DHL - Skillet - added !currentPlayerName because if the player was previously in freelook GetSpectatorTarget() returns 0
			//And an assert was getting hit inside GetPlayerName(), so this fixes it.
			if ( !currentPlayerName || !FStrEq( currentPlayerName, player ) ) 
			{
				char command[128];
...
void CSpectatorMenu::ShowPanel(bool bShow)
{
	...
	ConVarRef forcecamera( "mp_forcecamera" );
	m_pViewOptions->GetMenu()->SetItemVisible( "#Spec_Mode5", true );
	if ( pLocalPlayer && pLocalPlayer->GetTeamNumber() != TEAM_SPECTATOR && DHLRules()->IsTeamplay() )
	{
		if ( forcecamera.GetInt() == OBS_ALLOW_NONE )
			m_pViewOptions->SetVisible( false );

		if ( forcecamera.GetInt() == OBS_ALLOW_TEAM )
			m_pViewOptions->GetMenu()->SetItemVisible( "#Spec_Mode5", false );
	}
}
...
void CSpectatorMenu::Update( void )
{
	...
	for ( iPlayerIndex = 1 ; iPlayerIndex <= gpGlobals->maxClients; iPlayerIndex++ )
	{
		...
		if ( pLocalPlayer )
		{
			if ( pLocalPlayer->GetTeamNumber() != TEAM_SPECTATOR )
			{
				...
				if ( forcecamera.GetInt() == OBS_ALLOW_TEAM )
				{
					if ( gr->GetTeam( iPlayerIndex ) != pLocalPlayer->GetTeamNumber() )
						continue;
				}
			}
		}
	...
	//DHL - Skillet - runs into assert if in freelook
	const char *selectedPlayerName = playernum ? gr->GetPlayerName( playernum ) : "None";
