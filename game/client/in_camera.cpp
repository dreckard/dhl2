...
//DHL - Skillet
extern kbutton_t in_povchange;

//extern const ConVar *sv_cheats; //DHL - Commented
...
void CInput::CAM_Think( void )
{
	...
	// DHL: Toggle first/3rd person
	// Toggle first/3rd person, check every 0.5 seconds
	// If it's been half a second since our last change or the time we've got for the next change is just
	// Way out in left field (may happen on map change, camera is not reinitialized), proceed

	static float mf_NextCamSwitch = gpGlobals->curtime;
	if(gpGlobals->curtime >= mf_NextCamSwitch || (mf_NextCamSwitch - gpGlobals->curtime) >= 1.0f)
	{
		if(input->GetButtonBits(0) & IN_POVCHANGE)
		{
			//DHL - Don't let them switch perspective if they're dead.
			C_BasePlayer *Player = C_BasePlayer::GetLocalPlayer();
			if ( Player && Player->IsAlive() )
			{
				mf_NextCamSwitch = gpGlobals->curtime + 0.5; // Check if the key has been pressed every 0.5 seconds
	...
	//DHL - Skillet - Third person is allowed without cheats
	//if ( !sv_cheats )
	//{
...
{ 
	...
	if ( pPlayer )
	{
		pPlayer->PerspectiveChanged( PCHANGE_TOTHIRDPERSON );
	}
...
static ConCommand thirdperson( "thirdperson", ::CAM_ToThirdPerson, "Switch to thirdperson camera." ); //DHL Removed FCVAR_CHEAT
