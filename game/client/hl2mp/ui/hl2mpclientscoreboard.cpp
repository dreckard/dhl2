...
CHL2MPClientScoreBoardDialog::CHL2MPClientScoreBoardDialog( IViewPort *pViewPort ) : CClientScoreBoardDialog( pViewPort )
{
	...
	m_pWinsLabel_R = new Label( this, "R_Wins", "" ); //DHL - Skillet
	...
	m_pWinsLabel_C = new Label( this, "C_Wins", "" ); //DHL - Skillet
...
void CHL2MPClientScoreBoardDialog::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	m_pWinsHeader_DM = (Label*)FindChildByName( "DM_WinsHeader" ); //DHL - Skillet
	m_pDeathsHeader_DM = (Label*)FindChildByName( "DM_DeathsHeader" );
	m_pPingHeader_DM = (Label*)FindChildByName( "DM_PingHeader" );

	if ( m_pPlayerCountLabel_DM && m_pScoreHeader_DM && m_pDeathsHeader_DM && m_pPingHeader_DM && m_pPingLabel_DM && m_pWinsHeader_DM )
	{
		m_pPlayerCountLabel_DM->SetFgColor( COLOR_YELLOW );
		m_pScoreHeader_DM->SetFgColor( COLOR_YELLOW );
		m_pWinsHeader_DM->SetFgColor( COLOR_YELLOW ); //DHL - Skillet
	...
	m_pWinsHeader_R = (Label*)FindChildByName( "R_WinsHeader" ); //DHL - Skillet
	m_pDeathsHeader_R = (Label*)FindChildByName( "R_DeathsHeader" );
	m_pPingHeader_R = (Label*)FindChildByName( "R_PingHeader" );

	if ( m_pPlayerCountLabel_R && m_pScoreHeader_R && m_pScoreLabel_R && m_pDeathsHeader_R && m_pPingHeader_R && m_pPingLabel_R && m_pWinsHeader_R && m_pWinsLabel_R )
	{
		m_pPlayerCountLabel_R->SetFgColor( COLOR_RED );
		m_pScoreHeader_R->SetFgColor( COLOR_RED );
		m_pScoreLabel_R->SetFgColor( COLOR_RED );
		m_pWinsHeader_R->SetFgColor( COLOR_RED ); //DHL - Skillet
		m_pWinsLabel_R->SetFgColor( COLOR_RED ); //DHL - Skillet
	...
	m_pWinsHeader_C = (Label*)FindChildByName( "C_WinsHeader" ); //DHL - Skillet
	m_pDeathsHeader_C = (Label*)FindChildByName( "C_DeathsHeader" );
	m_pPingHeader_C = (Label*)FindChildByName( "C_PingHeader" );

	if ( m_pPlayerCountLabel_C && m_pScoreHeader_C && m_pScoreLabel_C && m_pDeathsHeader_C && m_pPingHeader_C && m_pPingLabel_C && m_pWinsHeader_C && m_pWinsLabel_C )
	{
		m_pPlayerCountLabel_C->SetFgColor( COLOR_BLUE );
		m_pScoreHeader_C->SetFgColor( COLOR_BLUE );
		m_pScoreLabel_C->SetFgColor( COLOR_BLUE );
		m_pWinsHeader_C->SetFgColor( COLOR_BLUE ); //DHL - Skillet
		m_pWinsLabel_C->SetFgColor( COLOR_BLUE ); //DHL - Skillet
...
bool CHL2MPClientScoreBoardDialog::HL2MPPlayerSortFunc( vgui::SectionedListPanel *list, int itemID1, int itemID2 )
{
	...
	//DHL - Skillet - wins first
	int v1 = it1->GetInt( "wins" );
	int v2 = it2->GetInt( "wins" );
	if ( v1 > v2 )
		return true;
	else if ( v1 < v2 )
		return false;
...
void CHL2MPClientScoreBoardDialog::InitPlayerList( SectionedListPanel *pPlayerList, int teamNumber )
{
	...
	if ( DHLRules() && DHLRules()->IsRoundplay() && !DHLRules()->IsTeamplay() ) //There's no "i" in team (but there is a "me")
		pPlayerList->AddColumnToSection( 0, "wins", "", SectionedListPanel::COLUMN_RIGHT, m_iWinsWidth ); //DHL - Skillet
	else
		pPlayerList->AddColumnToSection( 0, "bleh", "", SectionedListPanel::COLUMN_RIGHT, m_iWinsWidth ); //DHL - Skillet
...
void CHL2MPClientScoreBoardDialog::UpdateTeamInfo()
{
	...
	for ( int teamIndex = startTeam; teamIndex <= TEAM_REBELS; teamIndex++ )
	{
		...
		if ( team )
		{
			...
			const char *pDialogVarTeamWins = NULL; //DHL - Skillet
			switch ( teamIndex ) 
			{
				case TEAM_REBELS:
					...
					pDialogVarTeamWins = "r_teamwins"; //DHL - Skillet
					break;
				case TEAM_COMBINE:
					...
					pDialogVarTeamWins = "c_teamwins"; //DHL - Skillet
					break;
			...
			if ( teamIndex != TEAM_UNASSIGNED )	// Don't accumulate deathmatch scores.
			{
				...
				SetDialogVariable( pDialogVarTeamWins, team->GetRoundsWon() ); //DHL - Skillet
			}
...
bool CHL2MPClientScoreBoardDialog::GetPlayerScoreInfo( int playerIndex, KeyValues *kv )
{
	...
	kv->SetInt( "wins", g_PR->GetPlayerWins( playerIndex ) ); //DHL - Skillet
...
void CHL2MPClientScoreBoardDialog::UpdateItemVisibiity()
{
	...
	if ( HL2MPRules()->IsTeamplay() )
	{
		...
		{
			m_pWinsLabel_R->SetVisible( true );
			m_pWinsHeader_R->SetVisible( true );
		}
		else
		{
			m_pWinsLabel_R->SetVisible( false );
			m_pWinsHeader_R->SetVisible( false );
		}
		...
		if ( DHLRules()->IsRoundplay() )
		{
			m_pWinsLabel_C->SetVisible( true );
			m_pWinsHeader_C->SetVisible( true );
		}
		else
		{
			m_pWinsLabel_C->SetVisible( false );
			m_pWinsHeader_C->SetVisible( false );
		}
		...
		m_pWinsHeader_DM->SetVisible( false ); //DHL - Skillet
	...
	else
	{
		...
		m_pWinsHeader_R->SetVisible( false ); //DHL - Skillet
		m_pWinsLabel_R->SetVisible( false ); //DHL - Skillet
		...
		m_pWinsHeader_C->SetVisible( false ); //DHL - Skillet
		m_pWinsLabel_C->SetVisible( false ); //DHL - Skillet
		...
		//DHL - Skillet
		if ( DHLRules()->IsRoundplay() )
			m_pWinsHeader_DM->SetVisible( true );
		else
			m_pWinsHeader_DM->SetVisible( false );
