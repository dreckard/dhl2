...
void CHudDeathNotice::Paint()
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		if( g_PR )
		{
			//DHL - Skillet - Check to avoid assert in GetTeam()
...
void CHudDeathNotice::FireGameEvent( IGameEvent * event )
{
	...
	//DHL - Skillet - Vanilla code hits an assert in GetPlayerName if the player is killed by the world (falling damage) as killer = 0
