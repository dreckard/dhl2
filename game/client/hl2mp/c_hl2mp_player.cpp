...
#ifdef USE_FMOD
extern FMOD::DSP* locationalUnderwaterDSP;
#endif
...
//DHL - Skillet - We use C_DHL_Player now (which is derived from C_HL2MP_Player...)
#ifndef DISTRACTION
...
extern void FX_DustImpact( const Vector &origin, trace_t *tr, float flScale, Vector& color ); //DHL - Skillet - For blood

#ifdef USE_FMOD
FMOD::DSP* underwaterDSP;
#endif
...
{
	//DHL - Skillet
	m_bUnderwaterSoundEffects = false;
	m_vecClientViewPos = vec3_origin;
	m_pDeathViewEnt = NULL;
	m_flDeathViewEntEndTime = -1.0f;

#ifdef USE_FMOD
	//Underwater DSP effect
	DHLShared::GetFMODSystem()->createDSPByType( FMOD_DSP_TYPE_LOWPASS, &underwaterDSP );
	underwaterDSP->setParameter( 0, 3000.0f ); //Frequency Cutoff
	underwaterDSP->setParameter( 1, 1.02f ); //Resonance
	DHLShared::GetChannelGroups()->pChanGameMaster->addDSP( underwaterDSP );
	underwaterDSP->setBypass( true );
#endif
...
#ifdef USE_FMOD
	if ( IsLocalPlayer() )
		//DHL - Skillet - Argh, the DSP seems to get stuck on if a level change occurs while underwater
		underwaterDSP->setBypass( true );
#endif
}
...
void C_HL2MP_Player::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if ( m_takedamage )
	{
		...
		if ( blood != DONT_BLEED )
		{
			//DHL - Skillet - New blood puffs
			//SpawnBlood( vecOrigin, vecDir, blood, flDistance );// a little surface blood.
			//FX_DustImpact( vecOrigin, ptr, dhl_bloodpuffscale.GetFloat(), Vector( 72.0f/255.0f, 0, 0 ) );
			UTIL_BloodDrips( vecOrigin, vecDir, BLOOD_COLOR_RED, 50 );
...
{
	//DHL - Skillet
	bool bClientViewUnderwater = false;
	if ( !IsAlive() )
	{
		//No need to recalculate water state if we're roaming, GetWaterLevel() should be accurate
		if ( GetSpectatorMode() == OBS_MODE_ROAMING )
			bClientViewUnderwater = (GetWaterLevel() == 3);
		//If we're dead and lacking an observer mode, we're in ragdoll deathcam view
		else
			if ( enginetrace->GetPointContents( m_vecClientViewPos ) & MASK_WATER )
				bClientViewUnderwater = true;
	}

	if ( ( (IsAlive() && GetWaterLevel() == 3) || (!IsAlive() && bClientViewUnderwater) ) && !m_bUnderwaterSoundEffects )
	{
		CPASAttenuationFilter filter( this );

		EmitSound_t ep;
		ep.m_pSoundName = "Player.AmbientUnderWater";
		ep.m_nSpeakerEntity = entindex();
		ep.m_bLooping = true;
		EmitSound( filter, entindex(), ep );

#ifdef USE_FMOD
		//Apply DSP to all sounds
		underwaterDSP->setBypass( false );

		//Disable locational underwater DSP, so underwater sounds aren't lowpassed twice
		locationalUnderwaterDSP->setBypass( true );
#endif

		m_bUnderwaterSoundEffects = true;
	}
	else if ( ( (IsAlive() && GetWaterLevel() < 3) || (!IsAlive() && !bClientViewUnderwater) ) && m_bUnderwaterSoundEffects )
	{
		StopSound( "Player.AmbientUnderWater" );

		#ifdef USE_FMOD
			//Remove DSP
			//underwaterDSP->remove();
			underwaterDSP->setBypass( true );
			locationalUnderwaterDSP->setBypass( false );
		#endif
...
void C_HL2MP_Player::ItemPreFrame( void )
{
	...
	if ( m_nButtons & IN_ZOOM && m_HL2Local.m_bZooming ) //DHL - Skillet - Only if actually zoomed
	{
...
Vector vecLastOrigin = vec3_origin;
void C_HL2MP_Player::CalcView( Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar, float &fov )
{
	if ( m_lifeState != LIFE_ALIVE && !IsObserver() )
	{
		...
		//DHL - Skillet
		if ( m_pDeathViewEnt && (m_flDeathViewEntEndTime < 0.0f || m_flDeathViewEntEndTime > gpGlobals->curtime) )
		{
			origin = m_pDeathViewEnt->GetAbsOrigin();
			origin.z += VEC_DEAD_VIEWHEIGHT.z;
			vecLastOrigin = origin;
		}
		else if ( m_pDeathViewEnt != NULL )
		{
			origin = vecLastOrigin; //Last origin
		}
		else
		{
		...
		m_vecClientViewPos = eyeOrigin; //DHL - Skillet
		return;
	}
...
void C_HL2MPRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, char *pCustomImpactName )
{
	...
	if ( iDamageType == DMG_BLAST )
	{
		...
		//DHL: Changed from ApplyForceCenter to ApplyForceOffset
		pPhysicsObject->ApplyForceOffset( dir, pTrace->endpos );
	}
	else
	{
		Vector hitpos;  
		...
		// Blood spray!
		//DHL: This emits a little blood puff from ragdolls when they're shot
		//SpawnBlood( hitpos, (dir / 4000), BLOOD_COLOR_RED, 5.0f );
		//The color should be from global define in effect_color_tables.h but it's too much of a pain
		//The function likes color 0.0-1.0, hence the division
		FX_DustImpact( hitpos, pTrace, 0.5, Vector( 72.0f/255.0f, 0, 0 ) );
	}
...
void C_HL2MPRagdoll::CreateHL2MPRagdoll( void )
{
	...
	{
		//DHL: Setup the gravity for ragdolls
		SetGravity( pPlayer->GetGravity() );
	...
	/*{
		...
		pFlame->m_hEntAttached = this;
		pFlame->InitializeAsClientEntity( NULL, RENDER_GROUP_OPAQUE_ENTITY );
		pFlame->OnDataChanged( DATA_UPDATE_CREATED );
		pFlame->AddEffects( EF_NODRAW );
	}*/
	//Ignite( 10.0f, false );
	
	//DHL - Add this ragdoll to the bottom of our list if it's for a remote player.
	//Local player ragdolls don't count towards the maximum
	if ( m_hPlayer.Get() != C_BasePlayer::GetLocalPlayer() )
	{
		DHLRules()->m_hRagdollList.AddToHead( this );
	...
	//DHL - If we've got more ragdolls than allowed, remove the oldest one until we get down to an acceptable number
	if ( cl_ragdoll_maxcount.GetInt() >= 0 )
	{
		while ( DHLRules()->m_hRagdollList.Count() > cl_ragdoll_maxcount.GetInt() )
		{
			//Remove the ragdoll on the top of the list (oldest one)
			int iLastIndex = DHLRules()->m_hRagdollList.Count() - 1;
			if ( DHLRules()->m_hRagdollList.IsValidIndex( iLastIndex ) )
			{
				C_HL2MPRagdoll* Ragdoll = DHLRules()->m_hRagdollList.Element( iLastIndex );
				Ragdoll->Remove();
			}
		}
	}
...
{
	DHLRules()->m_hRagdollList.FindAndRemove( this ); //DHL
