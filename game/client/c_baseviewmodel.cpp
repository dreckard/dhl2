...
#if 1 //defined( CSTRIKE_DLL ) || defined( SDK_DLL ) //DHL - Skillet
...
//Tony; changed for SDK so that the CSS models can be flipped out of the box.
	...
	//DHL - Skillet - Flipping for akimbo and such
	//Mixed Akimbo
	/*CBaseCombatWeapon *pVMWeapon = m_hWeapon.Get();
	CDHLBaseWeapon *pDHLWep = dynamic_cast<CDHLBaseWeapon*>(pVMWeapon);
	if ( pDHLWep )
		return pDHLWep->NeedsFlip();
	CDHLBaseWeapon* pFlipWep = NULL;
	if ( pDHLWep && pDHLWep->IsAkimbo() )
	{
		if ( pDHLWep->GetAkimboPos() == AKIMBOPOS_RIGHT )
			pFlipWep = pDHLWep->m_pOtherAkimbo;
		else
			pFlipWep = pDHLWep;
	}
	if ( pFlipWep )
		return pFlipWep->NeedsFlip();*/
