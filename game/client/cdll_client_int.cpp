...
//Begin big DHL block
//----------------------------------------------
//Find the first command in this string from character number iPos
//Returns NULL if none found
//If successful, the index of the character after the command's end character is copied to iCmdEndPos, for iteration
const char* FindCommand( const char* szStr, int iPos, int *iCmdEndPos )
{
	if ( !szStr )
		return NULL;
	...
	std::string::size_type iRetTmp = 0;
	iRetTmp = str.find_first_not_of( " ;", iPos );
	if ( iRetTmp != npos )
	{
		std::string::size_type iStartPos = iRetTmp;
		iRetTmp = str.find_first_of( ";", iStartPos );
		if ( iRetTmp != npos )
		{
			*iCmdEndPos = iRetTmp;
			return str.substr( iStartPos, iRetTmp - iStartPos ).c_str();
		}
	}
	return NULL;
}

//List o' banned commands
//How this list works:
//Users are not allowed to have more than one of these commands in a bind or an alias
const char *pProtectedCommands[] =
{
	"+stunt",
	"+attack",
	"+attack2",
...
FnCommandCallback_t fnOrigAliasCallback;
FnCommandCallback_t fnOrigBindCallback;
ConCommand* pAlias = NULL;
ConCommand* pBind = NULL;
//DHL Alias command hook
void CC_HookAlias( const CCommand &command )
{
	if ( pAlias == NULL )
		return;
	...
	{
		char pArgText[128] = "";
		Q_strncpy( pArgText, command.Arg(2), 128 );
		strlwr(pArgText);

		//Semi colon is the delimiter for multiple commands
		if ( Q_strstr( pArgText, ";" ) )
		{
			bool bFoundCommand = false;
			int iPos = 0;
			int iCmdEndPos = 0;
			while ( FindCommand( pArgText, iPos, &iCmdEndPos ) )
			{
				for ( int i = 0; i <= ARRAYSIZE(pProtectedCommands); i++ )
				{
					if ( !pProtectedCommands[i] )
						continue;

					if ( Q_stristr( pArgText, pProtectedCommands[i] ) != NULL )
					{
						if ( !bFoundCommand )
							bFoundCommand = true;
						else
						{
							Msg( "Forbidden aliasing attempted: \"%s\"\n", pArgText );
							return;
						}
					}
				}
				iPos = iCmdEndPos; //Go to the end of the command we just checked
			}
		}
	}
	//Then call the engine's callback for alias
	pAlias->SetCommandCallback( fnOrigAliasCallback );
	pAlias->Dispatch( command );

	//Back to ours
	pAlias->SetCommandCallback(FnCommandCallback_t(CC_HookAlias));
}
void CC_HookBind( const CCommand &command )
{
	if ( pBind == NULL )
		return;
			...
			while ( FindCommand( pArgText, iPos, &iCmdEndPos ) )
			{
				for ( int i = 0; i <= ARRAYSIZE(pProtectedCommands); i++ )
				{
					...
					if ( Q_stristr( pArgText, pProtectedCommands[i] ) != NULL )
					{
						...
						{
							Msg( "Forbidden binding attempted: \"%s\"\n", pArgText );
							return;
						}
					}
				}
	...
	//Then call the engine's callback for alias
	pBind->SetCommandCallback( fnOrigBindCallback );
	pBind->Dispatch( command );

	//Back to ours
	pBind->SetCommandCallback(FnCommandCallback_t(CC_HookBind));
}
//----------------------------------------------
//End big DHL block
...
int CHLClient::Init( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals )
{
	...
	//DHL - Skillet - Anti-scripting crap
	//Alias stuffs
	pAlias = g_pCVar->FindCommand("alias");
	fnOrigAliasCallback = pAlias->GetCommandCallback();
	pAlias->SetCommandCallback(FnCommandCallback_t(CC_HookAlias));

	//Bind stuffs
	pBind = g_pCVar->FindCommand("bind");
	fnOrigBindCallback = pBind->GetCommandCallback();
	pBind->SetCommandCallback(FnCommandCallback_t(CC_HookBind));
...
void CHLClient::LevelInitPostEntity( )
{
	...
	//DHL - Skillet
	DHLShared::LevelInitPostEntity();

	//DHL MOTD Handler
	/*if ( !engine->IsLevelMainMenuBackground() )
	{
		//DHL - Skillet - Stop our menu music.
		#ifdef USE_FMOD
			DHLShared::ChannelGroupSetPaused( DHLShared::GetChannelGroups()->pChanBGMusic, true );
		#endif
		...
		IViewPortPanel* pMOTDPanel = gViewPortInterface->FindPanelByName( PANEL_INFO );
		if ( pMOTDPanel )
		{
			pMOTDPanel->SetData( data );
			pMOTDPanel->ShowPanel( true );
		}
		else
			Warning( "No MOTD panel \n");
...
void CHLClient::LevelShutdown( void )
{
	...
	//DHL - Skillet
#ifdef USE_FMOD
	//Stop all ingame sounds (weapons, etc)
	DHLShared::GetChannelGroups()->pChanGameMaster->stop();

	//Restart our BG music
	DHLShared::ChannelGroupSetPaused( DHLShared::GetChannelGroups()->pChanBGMusic, false );
#endif
