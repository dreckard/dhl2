...
void CHud::Init( void )
{
	...
	//DHL
	pDrawHUD = cvar->FindVar( "cl_drawhud" );
}
...
void CHud::LevelInit( void )
{
	...
	{
		//Save their original cl_drawhud setting
		iOriginalDrawHUD = pDrawHUD->GetInt();
		pDrawHUD->SetValue( 0 );
	}
	else
		//Use -1 as the value if we didn't load a BG map
		iOriginalDrawHUD = -1;
}
...
void CHud::LevelShutdown( void )
{
	...
	//DHL: Reset their cl_drawhud
	if ( iOriginalDrawHUD != -1 )
		pDrawHUD->SetValue( iOriginalDrawHUD );
}
