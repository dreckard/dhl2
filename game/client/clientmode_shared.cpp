...
ConVar cl_drawhud( "cl_drawhud","1", /* DHL Removed cheat flag */ 0, "Enable the rendering of the hud" );
...
void ClientModeShared::OverrideView( CViewSetup *pSetup )
{
	...
	if( ::input->CAM_IsThirdPerson() )
	{
		...
		//DHL - Skillet - "Over the shoulder" view
		Vector xDir = camRight;
		//const float xOffsetDist = 15.0f;
		const float yOffsetDist = 20.0f;

		//Test right
		/*trace_t viewTr;
		//UTIL_TraceLine( pPlayer->EyePosition(), pPlayer->EyePosition() + ( xOffsetDist * xDir ), MASK_ALL, NULL, COLLISION_GROUP_NONE, &viewTr );
		UTIL_TraceHull( pSetup->origin, pSetup->origin + ( xOffsetDist * xDir ), VEC_DUCK_HULL_MIN, 
			VEC_DUCK_HULL_MAX, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &viewTr );
		if ( ( viewTr.DidHit() && viewTr.fraction <= 0.1f ) || viewTr.allsolid )
		{
			//Right has a close obstacle, try the left
			xDir = -camRight;
			UTIL_TraceHull( pSetup->origin, pSetup->origin + ( xOffsetDist * xDir ), VEC_DUCK_HULL_MIN, 
				VEC_DUCK_HULL_MAX, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &viewTr );
			if ( ( viewTr.DidHit() && viewTr.fraction <= 0.1f ) || viewTr.allsolid )
				xDir = vec3_origin; //Still no good, kill the offset
		}*/
		//pSetup->origin += ( xOffsetDist * xDir );
		pSetup->origin += ( yOffsetDist * camUp );

		//Check on the surroundings
		trace_t tr;
		UTIL_TraceHull( pPlayer->EyePosition(), pSetup->origin, VEC_DUCK_HULL_MIN, 
				VEC_DUCK_HULL_MAX, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr );
		if ( tr.DidHit() )
		{
			pSetup->origin = tr.endpos;

			C_BaseCombatWeapon *pWeapon = pPlayer->GetActiveWeapon();
			if ( tr.startpos.DistTo( tr.endpos ) <= 10.0f )
			{
				pPlayer->SetRenderMode( kRenderTransColor );
				pPlayer->SetRenderColorA( 50 );

				if ( pWeapon )
				{
					pWeapon->SetRenderMode( kRenderTransColor );
					pWeapon->SetRenderColorA( 50 );
				}
			}
			else
			{
		...
		//DHL - Skillet - Apply viewpunches to third person
		if ( pPlayer->m_Local.m_vecPunchAngleVel.Get() != vec3_angle )
		{
			camAngles[PITCH] += pPlayer->m_Local.m_vecPunchAngle[PITCH];
			camAngles[YAW] += pPlayer->m_Local.m_vecPunchAngle[YAW];
			camAngles[ROLL] += pPlayer->m_Local.m_vecPunchAngle[ROLL];
		}
...
int	ClientModeShared::KeyInput( int down, ButtonCode_t keynum, const char *pszCurrentBinding )
{
				  ...
				  Q_strcmp( pszCurrentBinding, "say_team" ) == 0 ) )
	{
		if ( down )
		{
			if ( DHLRules() && DHLRules()->IsTeamplay() ) //DHL - Skillet - Only allow teamchat in teamplay (duh)
...
{
	//DHL - Skillet - Necessary for spawning on +attack to work
	/*C_DHL_Player* pDHLPlayer = ToDHLPlayer(C_BasePlayer::GetLocalPlayer());
	if ( pDHLPlayer )
	{
		if ( pDHLPlayer->GetTeamNumber() != TEAM_SPECTATOR && DHLRules()->AllowRespawn() )
			return 1;
	}*/
	...
	else if ( down && pszCurrentBinding && Q_strcmp( pszCurrentBinding, "+attack" ) == 0 )
	{
		...
		//DHL - Skillet
		//Hack: process the key if not spectator (i.e. dead & in observer mode but on a team) because we may need to respawn on that signal
		C_DHL_Player* pDHLPlayer = ToDHLPlayer(C_BasePlayer::GetLocalPlayer());
		if ( pDHLPlayer && pDHLPlayer->GetTeamNumber() != TEAM_SPECTATOR )
			return 1;
...
void ClientModeShared::FireGameEvent( IGameEvent *event )
{
	...
	if ( Q_strcmp( "player_connect", eventname ) == 0 )
	{
		...
		if ( !IsInCommentaryMode() )
		{
			...
			g_pVGuiLocalize->ConstructString( wszLocalized, sizeof( wszLocalized ), g_pVGuiLocalize->Find( "#Game_connected" ), 1, wszPlayerName ); //DHL - Skillet - was #game_player_joined_game 
	...
	else if ( Q_strcmp( "player_team", eventname ) == 0 )
	{
		...
		if ( !bSilent )
		{
			...
			if ( !IsInCommentaryMode() )
			{
				...
				{
					//DHL - Skillet
					if ( team == TEAM_UNASSIGNED && DHLRules() && !DHLRules()->IsTeamplay() )
						g_pVGuiLocalize->ConstructString( wszLocalized, sizeof( wszLocalized ), g_pVGuiLocalize->Find( "#DHL_PLAYER_JOINED_DEATHMATCH" ), 2, wszPlayerName, wszTeam );
					else
	...
	//DHL - Skillet
	else if ( !Q_strcmp( "player_spawn", eventname ) )
	{
		C_DHL_Player *pLocalPlayer = ToDHLPlayer(CBasePlayer::GetLocalPlayer());
		if ( !pLocalPlayer )
			return;
		if ( USERID2PLAYER( event->GetInt("userid") ) == pLocalPlayer )
		{
			pLocalPlayer->ClientRespawn();
		}
	}
