...
//DHL
static  kbutton_t   in_stunt;
static	kbutton_t	in_bandage;
static	kbutton_t	in_povchange;
...
//DHL Stunts Yet Again
void IN_StuntDown( const CCommand &args ) {KeyDown(&in_stunt, args[1] );}
void IN_StuntUp( const CCommand &args ) {KeyUp(&in_stunt, args[1] );}
//DHL First/Thirdperson Toggler:
void IN_PovDown( const CCommand &args ) {KeyDown(&in_povchange, args[1] );}
void IN_PovUp( const CCommand &args ) {KeyUp(&in_povchange, args[1] );}
//DHL Bandaging Key
void IN_BandageDown( const CCommand &args ) {KeyDown(&in_bandage, args[1] );}
void IN_BandageUp( const CCommand &args ) {KeyUp(&in_bandage, args[1] );}
...
int CInput::GetButtonBits( int bResetState )
{
	...
	// DHL:(CloudHackIX) Missed a spot there Skillet
    CalcButtonBits( bits, IN_STUNT, s_ClearInputState, &in_stunt, bResetState );
	// DHL POV Toggler:
	CalcButtonBits( bits, IN_POVCHANGE, s_ClearInputState, &in_povchange, bResetState );
	// DHL Bandaging Key:
	CalcButtonBits( bits, IN_BANDAGE, s_ClearInputState, &in_bandage, bResetState );
...
//DHL Stunts
static ConCommand startstunt("+stunt",IN_StuntDown);
static ConCommand endstunt("-stunt",IN_StuntUp);
//DHL POV Toggle:
static ConCommand startpovchange("+povchange",IN_PovDown);
static ConCommand endpovchange("-povchange", IN_PovUp);
//DHL Bandage Key:
static ConCommand startbandage("+bandage",IN_BandageDown);
static ConCommand endbandage("-bandage", IN_BandageUp);
