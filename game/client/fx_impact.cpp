...
IterationRetval_t CRagdollEnumerator::EnumElement( IHandleEntity *pHandleEntity )
{
	...
	if ( tr.fraction < 1.0 )
	{
		//DHL - Skillet - Check our skip list for this entity
		if ( m_pSkipList )
			if ( m_pSkipList->Find( pEnt ) != m_pSkipList->InvalidIndex() )
				return ITERATION_CONTINUE;

		pModel->ImpactTrace( &tr, m_iDamageType, NULL );
		m_bHit = true;
		m_pEnt = pEnt; //DHL - Skillet
...
//DHL - Skillet
CBaseEntity* DHL_FX_AffectRagdolls( Vector vecOrigin, Vector vecStart, int iDamageType, CUtlVector<CBaseEntity*>* pSkipList )
{
	// don't do this when lots of ragdolls are simulating
	if ( s_RagdollLRU.CountRagdolls(true) > 1 )
		return false;
	...
	CRagdollEnumerator ragdollEnum( shotRay, iDamageType );
	ragdollEnum.m_pSkipList = pSkipList;
	partition->EnumerateElementsAlongRay( PARTITION_CLIENT_RESPONSIVE_EDICTS, shotRay, false, &ragdollEnum );

	return ragdollEnum.m_pEnt;
}
