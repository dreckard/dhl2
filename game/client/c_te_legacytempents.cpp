...
void CTempEnts::EjectBrass( const Vector &pos1, const QAngle &angles, const QAngle &gunAngles, int type )
{
	...
	//DHL - Skillet - Scale for timescale
	pTemp->die = gpGlobals->curtime + (1.0f + random->RandomFloat( 0.0f, 1.0f )) / DHLRules()->GetTimescale();	// Add an extra 0-1 secs of life	
}
...
void CTempEnts::PlaySound ( C_LocalTempEntity *pTemp, float damp )
{
	...
	{	
		...
		//DHL - Skillet - Don't play very minor collisions
		if ( zvel < 25 )
			return;
	}
	else
...
void CTempEnts::Update(void)
{
	...
	//DHL - Skillet - Slow motion
	frametime *= DHLRules()->GetTimescale();
