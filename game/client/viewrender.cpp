...
//DHL - Skillet
ConVar dhl_grayscale("dhl_grayscale", "0", FCVAR_CHEAT, "Enable the grayscale shader" );
ConVar dhl_nightvision("dhl_nightvision", "0", FCVAR_CHEAT, "Enable the nightvision shader" );
...
//DHL - Skillet - Handles all of the overlay application stuff
ConVar dhl_slowmo_grayscale( "dhl_slowmo_grayscale", "1", FCVAR_CHEAT );
void CViewRender::DoDHLOverlays( void )
{
	CDHL_Player* pPlayer = ToDHLPlayer( CBasePlayer::GetLocalPlayer() );
	if ( pPlayer )
	{
		if ( pPlayer->m_bScoped )
			AddViewOverlay( 70, "dhl_scope" );

		//Switched over to the valve refract shader, looks better than my crap custom written one
		if ( pPlayer->blurInfo.bBlur )
		{
			if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 90 )
			{
				IMaterial *pMaterial = materials->FindMaterial( "dhl_shaders/dhl_blur", TEXTURE_GROUP_CLIENT_EFFECTS, true );
				//Shader reads these when rendering
				Assert( pMaterial );
				pMaterial->FindVar( "$refractamount", NULL )->SetFloatValue( 0.01f * ( 1 / pPlayer->blurInfo.flFraction ) );
				pMaterial->FindVar( "$refracttint", NULL )->SetVecValue( 1.0f, 1.0f * pPlayer->blurInfo.flFraction, 1.0f * pPlayer->blurInfo.flFraction );
				/*IMaterial *pMaterial = materials->FindMaterial( "dhl_shaders/dhl_blur", TEXTURE_GROUP_CLIENT_EFFECTS, true );
				//Shader reads these when rendering
				Assert( pMaterial );
				pMaterial->FindVar( "$SHIFTINTERVAL", NULL )->SetFloatValue( pPlayer->blurInfo.flInterval ); //ShiftInterval
				pMaterial->FindVar( "$UPDATEINTERVAL", NULL )->SetFloatValue( 0.025f ); //UpdateInterval - Kept constant for now
				pMaterial->FindVar( "$MIN", NULL )->SetFloatValue( pPlayer->blurInfo.flMin ); //Min
				pMaterial->FindVar( "$MAX", NULL )->SetFloatValue( pPlayer->blurInfo.flMax ); //Max
				pMaterial->FindVar( "$TIMESCALE", NULL )->SetFloatValue( DHLRules()->GetTimescale() ); //Timescale
				pMaterial->FindVar( "$GAMECURTIME", NULL )->SetFloatValue( gpGlobals->curtime ); //Curtime
				pMaterial->FindVar( "$FRACTION", NULL )->SetFloatValue( pPlayer->blurInfo.flFraction ); //Health / effects health*/

				AddViewOverlay( 90, &pMaterial );
			}
			else //Fallback screenfade for lower DX versions
			{
				vieweffects->ClearAllFades(); //Hackish, will kill any other fades
				float flScalar = ( 1.0f - pPlayer->blurInfo.flFraction );
				ScreenFade_t fade;
				fade.duration = 128; // 0.25 sec - these do a weird float->short conversion for networking, 1/512
				fade.holdTime = 512; // 1 sec
		...
		if ( dhl_nightvision.GetBool() || pPlayer->GetNightvisionEnabled() )
		{
			AddViewOverlay( 80, "dhl_shaders/dhl_nightvision" );
		}

		//Grayscale stuff
		if ( dhl_slowmo_grayscale.GetBool() && pPlayer->m_flLastTimescaleChange > 0.0f 
			&& DHLRules()->GetTimescale() <= 1.0f && !DHLRules()->GetGrayscale() )
		{
			float flTimeDiff = gpGlobals->curtime - pPlayer->m_flLastTimescaleChange;
			float flAlpha = DHLRules()->GetTimescale() < 1.0f ? 1.0f : 0.0f;
			if ( flTimeDiff < 1.0f )
			{
				flAlpha = min( flTimeDiff/0.25f, 1.0f );
			...
			if ( flAlpha > 0.0f )
			{
				...
				pMaterial->FindVar( "$GRAYSCALE_ALPHA", NULL )->SetFloatValue( flAlpha );
				AddViewOverlay( 80, &pMaterial );
			}
		}
		if ( dhl_grayscale.GetBool() || DHLRules()->GetGrayscale() )
		{
			IMaterial *pMaterial = materials->FindMaterial( "dhl_shaders/dhl_grayscale", TEXTURE_GROUP_CLIENT_EFFECTS, true );
			pMaterial->FindVar( "$GRAYSCALE_ALPHA", NULL )->SetFloatValue( 1.0f );
			AddViewOverlay( 80, &pMaterial );
		}
	}
}

//DHL - Skillet - Standard function for applying texture view overlays
void CViewRender::AddViewOverlay( int iMinDXLevel, const char *filename )
{
	if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= iMinDXLevel )
	{
		// Get whatever material references your postprocess shader.
		IMaterial *pMaterial = materials->FindMaterial( filename, TEXTURE_GROUP_CLIENT_EFFECTS, true );
		if ( pMaterial )
		{
			const CViewSetup* ViewSetup = view->GetViewSetup();
			//UpdateRefractTexture( ViewSetup->x, ViewSetup->y,ViewSetup->width, ViewSetup->height, true );
			...
			/*const CViewSetup* ViewSetup = view->GetViewSetup();
			UpdateScreenEffectTexture( 0, ViewSetup->x, ViewSetup->y, ViewSetup->width, ViewSetup->height, true );
...
//DHL - Skillet - Give your predefined material pointer (to allow for pre-rendering modifications)
void CViewRender::AddViewOverlay( int iMinDXLevel, IMaterial** pMaterial )
{
	if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= iMinDXLevel )
	{
		// Get whatever material references your postprocess shader.
