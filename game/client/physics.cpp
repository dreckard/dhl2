...
#endif
		physenv->Simulate( frametime * cl_phys_timescale.GetFloat() * DHLRules()->GetTimescale() ); //DHL - Skillet - GetTimescale()
...
ConVar dhl_cl_ragdollbleedvel( "dhl_cl_ragdollbleedvel", "200.0", FCVAR_ARCHIVE, "Adjusts the impact speed above which ragdolls will spawn blood decals." );
void CCollisionEvent::PostCollision( vcollisionevent_t *pEvent )
{
	...
	//DHL - Skillet
	//float flMinCollisionSpeed = 50.0f;
	if ( pEvent->collisionSpeed > dhl_cl_ragdollbleedvel.GetFloat() )
	{
		...
		for ( int i = 0; i < 2; i++ )
		{
			...
			C_HL2MPRagdoll* pRagdoll = dynamic_cast<C_HL2MPRagdoll*>(reinterpret_cast<C_BaseEntity*>(pObject->GetGameData()));
			if ( pRagdoll )
			{
				/*if ( pRagdoll->flLastDecalTime >= 0.0f )
				{
					if ( (gpGlobals->curtime - pRagdoll->flLastDecalTime) >= 1.0f )
						flMinCollisionSpeed = 225.0f;
					else
						flMinCollisionSpeed = 325.0f;
				}*/

				Vector objPos = vec3_origin;
				pObject->GetPosition( &objPos, NULL );

				trace_t tr;
				Vector vecEndPos = vecPos + 10.0f * (vecPos - objPos);
				UTIL_TraceLine( objPos, vecEndPos, MASK_SOLID, pRagdoll, COLLISION_GROUP_NONE, &tr );
				if ( tr.DidHit() )
				{
					UTIL_BloodDecalTrace( &tr, BLOOD_COLOR_RED );
					//pRagdoll->flLastDecalTime = gpGlobals->curtime;
				}
			}
		}
	}
