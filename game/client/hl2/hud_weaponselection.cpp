	...
	case HUDTYPE_BUCKETS:
		{
			...
			if ( pWeapon->GetSpriteActive() )
			{
				...
				//DHL - Skillet - Added HasAmmo()
				if (!pWeapon->CanBeSelected() || !pWeapon->HasAmmo() )
				{
...
C_BaseCombatWeapon *CHudWeaponSelection::GetWeaponInSlot( int iSlot, int iSlotPos )
{
	...
	for ( int i = 0; i < MAX_WEAPONS; i++ )
	{
		...
		//DHL - Skillet: There may be at some point in time more than one weapon in the player's inventory that has the same slot and position
		//Valve apparently found this inconceivable
		if ( !pWeapon->CanBeSelected() )
			continue;
	}
