...
bool CMultiPlayerAnimState::HandleProne( Activity &idealActivity )
{
	...
	{
		//Assert( g_pMoveData->m_nPlayerHandle == GetBasePlayer() );
		if ( pDHLPlayer->m_iStuntDir == STUNTDIR_FORWARDS &&
			//g_pMoveData->m_nPlayerHandle == GetBasePlayer() &&
			abs(pDHLPlayer->GetLocalVelocity().Length()) >= DHL_WALK_SPEED * DHL_PRONESPEED_SCALAR )
		{
...
bool CMultiPlayerAnimState::HandleSwimming( Activity &idealActivity )
{
	if ( GetBasePlayer()->GetWaterLevel() >= WL_Waist )
	{
		...
		//idealActivity = ACT_MP_SWIM;		//DHL - Skillet - We don't have swim anims
		m_bInSwim = true;
		return true;
	}
	else
...
Activity CMultiPlayerAnimState::CalcMainActivity()
{
	...
	if ( HandleProne( idealActivity ) || //DHL - Skillet
