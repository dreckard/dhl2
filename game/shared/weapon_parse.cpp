...
#include "hl2mp_gamerules.h" //DHL - Skillet
...
itemFlags_t g_ItemFlags[9] =
{
	...
	{ "ITEM_FLAG_NODROP",			ITEM_FLAG_NODROP } //DHL
};
#else
extern itemFlags_t g_ItemFlags[9];
#endif
...
bool ReadWeaponDataFromFileForSlot( IFileSystem* filesystem, const char *szWeaponName, WEAPON_FILE_INFO_HANDLE *phandle, const unsigned char *pICEKey )
{
	...
	*phandle = FindWeaponInfoSlot( szWeaponName );
	...
	char sz[128];
	Q_snprintf( sz, sizeof( sz ), "scripts/weapons/%s", szWeaponName ); //DHL - Moved weapon scripts to /weapons/ directory, for better organization
...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	//DHL - Skillet - Yes I know that the script keyvalue names and the variable names do not match.
	//It was decided that reconfiguring the names allowed for better usablity.
	//This basically turns the player's Z view plane into a traditional coordinate plane.
	fRecoilXMin = pKeyValuesData->GetFloat( "recoil_ymin", 0.0f );
	fRecoilXMax = pKeyValuesData->GetFloat( "recoil_ymax", 0.0f );
	fRecoilXMin = fRecoilXMin = -fRecoilXMin;
	fRecoilXMax = fRecoilXMax = -fRecoilXMax;

	fRecoilYMin = pKeyValuesData->GetFloat( "recoil_xmin", 0.0f );
	fRecoilYMax = pKeyValuesData->GetFloat( "recoil_xmax", 0.0f );
	fRecoilYMin = fRecoilYMin = -fRecoilYMin;
	fRecoilYMax = fRecoilYMax = -fRecoilYMax;

	flHeadDmgSclr = pKeyValuesData->GetFloat( "dmgsclr_head", 3.2f );
	flBodyDmgSclr = pKeyValuesData->GetFloat( "dmgsclr_body", 1.0f );
	flArmDmgSclr = pKeyValuesData->GetFloat( "dmgsclr_arm", 0.7 );
	flLegDmgSclr = pKeyValuesData->GetFloat( "dmgsclr_leg", 0.7 );

	fRoF = pKeyValuesData->GetFloat( "rof", 0.25f );

	flAccuracy = pKeyValuesData->GetFloat( "accuracy", 0.0f );
	flAccuracy2 = pKeyValuesData->GetFloat( "accuracy2", 0.0f );
	flMoveAccSclr = pKeyValuesData->GetFloat( "accuracy_movsclr", 0.5f );

	bCanAkimbo = pKeyValuesData->GetInt( "can_akimbo", 0 ) > 0;

	fMeleeRange = pKeyValuesData->GetFloat( "melee_range", 16.0f );
	fMeleeDamage = pKeyValuesData->GetFloat( "melee_damage", 0.0f );
	fMeleeHullW = pKeyValuesData->GetFloat( "melee_hull_w", 8.0f );
	fMeleeHullH = pKeyValuesData->GetFloat( "melee_hull_h", 8.0f );

	iDefaultAmmoPrimary = pKeyValuesData->GetInt( "defaultammo_primary", 0 );
	iDefaultAmmoSecondary = pKeyValuesData->GetInt( "defaultammo_secondary", 0 );
	iMaxAmmoPrimary = pKeyValuesData->GetInt( "maxammo_primary", 100 );
	CAmmoDef* pAmmoDef = GetAmmoDef();
	pAmmoDef->GetAmmoOfIndex( iAmmoType )->pMaxCarry = iMaxAmmoPrimary;
	iMaxAmmoSecondary = pKeyValuesData->GetInt( "maxammo_secondary", 0 );
	pAmmoDef->GetAmmoOfIndex( iAmmo2Type )->pMaxCarry = iMaxAmmoPrimary;
}
