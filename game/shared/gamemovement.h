...
//DHL - Skillet
//Ok, just including dhl_player_inc.h here causes a couple of completely nonsensical errors related to ladder code in hl_gamemovement.cpp
//I have no idea why.  This works fine.  Go figure.
#ifdef CLIENT_DLL
class C_DHL_Player;
...
protected:
	...
	//DHL - Skillet
	virtual bool	Stunt( void );
	virtual void	TryClimb( void );
	...
	//DHL - Skillet: Prone
	void Prone( bool bDown );
	void ProneDown( void );
	void ProneUp( void );
	void HandleProneSpeedCrop( void );
	void FixPlayerProneStuck( void );
	bool CanUnProne( void );

	//DHL - Skillet
	void			DHLThink( void );
	void			ProneThink( void );
