...
#include "dhl/dhl_gamerules.h" //DHL - Skillet
...
//DHL - Skillet - Made these cvars related to the sdk's roundplay CHEAT | HIDDEN for now so people don't mess with them
ConVar mp_restartgame( "mp_restartgame", "0", FCVAR_GAMEDLL | FCVAR_CHEAT | FCVAR_HIDDEN, "If non-zero, game will restart in the specified number of seconds" );

#ifndef TF_DLL		// TF overrides the default value of this convar
ConVar mp_waitingforplayers_time( "mp_waitingforplayers_time", "0", FCVAR_GAMEDLL | FCVAR_CHEAT | FCVAR_HIDDEN, "WaitingForPlayers time length in seconds" );
#endif

ConVar mp_waitingforplayers_restart( "mp_waitingforplayers_restart", "0", FCVAR_GAMEDLL | FCVAR_CHEAT | FCVAR_HIDDEN, "Set to 1 to start or restart the WaitingForPlayers period." );
ConVar mp_waitingforplayers_cancel( "mp_waitingforplayers_cancel", "0", FCVAR_GAMEDLL | FCVAR_CHEAT | FCVAR_HIDDEN, "Set to 1 to end the WaitingForPlayers period." );
ConVar mp_clan_readyrestart( "mp_clan_readyrestart", "0", FCVAR_GAMEDLL | FCVAR_CHEAT | FCVAR_HIDDEN, "If non-zero, game will restart once someone from each team gives the ready signal" );
ConVar mp_clan_ready_signal( "mp_clan_ready_signal", "ready", FCVAR_GAMEDLL | FCVAR_CHEAT | FCVAR_HIDDEN, "Text that team leader from each team must speak for the match to begin" );
...
{
#ifndef CLIENT_DLL
	//RefreshSkillData( true ); //DHL - Skillet - No skill.cfg execution
...
bool CMultiplayRules::Init()
{
	...
	void CMultiplayRules::PlayerKilled( CBasePlayer *pVictim, const CTakeDamageInfo &info )
	{
		...
		//DHL, don't score if the game isn't running
		...
		// DHL: No scoring if we're waiting for another player
		// Did the player kill himself?
		if ( pVictim == pScorer && !DHLRules()->IsGameWaiting() )
		{
		...
		{
			//DHL, once again, no scoring unless the game is running
			if ( !DHLRules()->IsGameWaiting() )
			{
		...
		{  
			if ( UseSuicidePenalty() && !DHLRules()->IsGameWaiting() ) //DHL
			{
