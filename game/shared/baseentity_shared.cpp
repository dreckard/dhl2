...
//DHL - Skillet
ConVar  dhl_penetrationscalar( "dhl_penetrationscalar", "1.0", FCVAR_CHEAT | FCVAR_REPLICATED, "Scale all penetration distances by this value" );
ConVar  dhl_materialspecificpenetration( "dhl_materialspecificpenetration", "1", FCVAR_REPLICATED, "Enable or disable penetration distance scaling by material" );
ConVar  dhl_distancescaledpenetration( "dhl_distancescaledpenetration", "1", FCVAR_REPLICATED, "Enable or disable penetration distance scaling by the bullet's distance travelled" );
ConVar  dhl_debugpenetration( "dhl_debugpenetration", "0", FCVAR_CHEAT | FCVAR_REPLICATED, "Turn on a multitude of console messages to help debug penetration." );
...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	// bullet's final direction can be changed by passing through a portal
		...
		//DHL: Our penetration handler makes HandleShotImpactingGlass obselete
		...
		if ( tr.fraction != 1.0 )
		{
			HandleShotPenetration( info, tr, vecDir, vecEnd, &traceFilter ); //DHL
		}
...
//-----------------------------------------------------------------------------
// DHL Penetration Handler, See CBaseEntity::HandleShotImpactingGlass
//-----------------------------------------------------------------------------
bool CBaseEntity::HandleShotPenetration( const FireBulletsInfo_t &info, const trace_t &tr, 
	 const Vector &vecDir, const Vector &vecEnd, ITraceFilter *pTraceFilter )
{
	CBasePlayer* pPlayer = ToBasePlayer(info.m_pAttacker);
	if ( !pPlayer )
		return false;

	PenetrationData_t nPenetrationData = DHLShared::TestPenetration( tr, pPlayer, pTraceFilter, info.m_iTimesPenetrated, 
		tr.startpos.DistTo( tr.endpos ), info.m_iAmmoType );
	if ( !nPenetrationData.m_bShouldPenetrate )
		return false;

	FireBulletsInfo_t penetrateInfo;
	//This is all going to get run for every bullet impact, so we don't need to worry about shotguns
	//And other multi-projectile per fire weapons
	//NOTENOTE: Now that we've implemented tracking for the number of penetrations for a single FireBullets() call, we need to
	//Use this information to determine when to increment m_iTimesPenetrated and m_flTotalDistancePenetrated
	//Save the original number and temporarily set it to 1 so we only penetrate 1 'bullet' at a time
	int iOrigShots = 0;
	iOrigShots = info.m_iShots;
	penetrateInfo.m_iShots = 1;

	//Definitely do not want to hit the entity we just penetrated again with the same bullet if it takes damage
	if ( tr.m_pEnt->m_takedamage == DAMAGE_YES )
		penetrateInfo.m_pAdditionalIgnoreEnt = tr.m_pEnt;

	penetrateInfo.m_vecSrc = nPenetrationData.m_vecNewBulletPos;
	penetrateInfo.m_vecDirShooting = vecDir;
	penetrateInfo.m_vecSpread = vec3_origin;
	penetrateInfo.m_flDistance = info.m_flDistance *( 1.0f - tr.fraction );
	penetrateInfo.m_iAmmoType = info.m_iAmmoType;
	penetrateInfo.m_iTracerFreq = info.m_iTracerFreq;
	penetrateInfo.m_flTotalDistancePenetrated = info.m_flTotalDistancePenetrated + tr.endpos.DistTo( nPenetrationData.m_vecNewBulletPos );
	penetrateInfo.m_iTimesPenetrated = info.m_iTimesPenetrated + 1;

	//if ( iOrigShots > 1 )
		//Remember, the cast from the float result to an int is a truncation, NOT rounding
		//penetrateInfo.m_iTimesPenetrated /= iOrigShots;

	//Damage and PlayerDamage are overridden later in the bullet firing process
	//Use m_flDamageScalar to change damage instead
	penetrateInfo.m_iDamage = info.m_iDamage;
	penetrateInfo.m_iPlayerDamage = info.m_iPlayerDamage;

	//Scale down damage
	penetrateInfo.m_flDamageScalar *= 0.85f;
#ifndef CLIENT_DLL
	if ( dhl_debugpenetration.GetBool() )
		Msg( "Damage scaled from %i to %f \n", penetrateInfo.m_iPlayerDamage, ( float(penetrateInfo.m_iPlayerDamage) * penetrateInfo.m_flDamageScalar ) );
#endif

	penetrateInfo.m_pAttacker = info.m_pAttacker;
	penetrateInfo.m_nFlags = info.m_nFlags;
	//Fires new bullet with same info and dir as the original from the end of the solid
	FireBullets( penetrateInfo );

	//Restore original m_iShots
	penetrateInfo.m_iShots = iOrigShots;
	return true;
}
