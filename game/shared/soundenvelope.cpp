...
public:
	...
	int m_iInitialPitch; //DHL - Skillet
...
bool CSoundPatch::Update( float time, float deltaTime )
{
	...
	if ( m_flags && m_Filter.IsActive() )
	{
		...
		ep.m_bFromSoundPatch = true; //DHL - Skillet
...
//	Msg( "Start sound %s\n", m_pszSoundName );
	...
	if ( m_Filter.IsActive() )
	{
		...
		ep.m_bFromSoundPatch = true; //DHL - Skillet
		m_iInitialPitch = ep.m_nPitch; //DHL - Skillet
...
public:
	...
	//DHL - Skillet
	int				GetInitialPitch( CSoundPatch *pSound );
...
private:
	//CUtlVector<CSoundPatch *>			m_soundList; //DHL - Skillet - Moved to CSoundEnvelopeController
...
//DHL - Skillet
int CSoundControllerImp::GetInitialPitch( CSoundPatch *pSound )
{
	return pSound->m_iInitialPitch;
}
