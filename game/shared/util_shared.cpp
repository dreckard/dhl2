...
////-----------------------------------------------------------------------------
//// DHL - Skillet -
//// Trace filter to hit only one entity (used to find hitbox)
////-----------------------------------------------------------------------------
CTraceFilterHitOneEntity::CTraceFilterHitOneEntity( const IHandleEntity *hitentity, int collisionGroup ) : CTraceFilterSimple( NULL, collisionGroup )
{
	m_pHitEntity = hitentity;
}
bool CTraceFilterHitOneEntity::ShouldHitEntity( IHandleEntity *pServerEntity, int contentsMask )
{
	if ( m_pHitEntity )
	{
		if ( pServerEntity == m_pHitEntity )
			return true;
	}
	return false;
}
...
void UTIL_BloodDecalTrace( trace_t *pTrace, int bloodColor, bool bSkipPredictionCull )
{
	if ( UTIL_ShouldShowBlood( bloodColor ) )
	{
		//DHL - Skillet - Reorganized this a bit
		const char* decalName;
		if ( bloodColor == BLOOD_COLOR_RED )
			decalName = "Blood";
		else
			decalName = "YellowBlood";
		//DHL - Skillet - We have to skip all the function calls and go straight to the temp ent system
#ifndef CLIENT_DLL
		if ( bSkipPredictionCull )
		{
			if ( pTrace->DidHit() )
			{
				...
				filter.SetIgnorePredictionCull( true ); //DHL - Skillet - Isn't networked otherwise
				if ( pTrace->hitbox != 0 )
				{
		...
		if ( bloodColor == BLOOD_COLOR_RED )
		{
			UTIL_DecalTrace( pTrace, decalName );
		}
		else
		{
			UTIL_DecalTrace( pTrace, decalName );
