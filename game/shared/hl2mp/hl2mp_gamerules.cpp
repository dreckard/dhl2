...
#include "ammodef.h"
	...
	#include "dhl/dhl_projectile.h" //DHL - Skillet
#ifdef DEBUG	
...
char *sTeamNames[] =
{
	"Unassigned",
	"Spectator",
	"Mobsters", //DHL; was "Combine"
	"Professionals", //DHL; was "Rebels"
};
	...
	else
	{
		...
		if ( pScorer )
		{
			...
			if ( pInflictor )
			{
				...
				{
					//DHL - Skillet - Read the weapon name stored in the bullet (a bit Judge Dredd-ish?)
					CDHLProjectile* pBullet = dynamic_cast<CDHLProjectile*>(pInflictor);
					if ( pBullet && pBullet->m_pFiringWeapon )
						killer_weapon_name = pBullet->m_pFiringWeapon->GetClassname();
					else
