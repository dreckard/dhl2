...
void CBaseHL2MPBludgeonWeapon::ItemPostFrame( void )
{
	...
	if ( (pOwner->m_nButtons & IN_ATTACK) && (m_flNextPrimaryAttack <= gpGlobals->curtime)
		&& ( m_iFireInputType == FIREINPUTTYPE_CANHOLD || pOwner->m_afButtonPressed & IN_ATTACK ) ) //DHL - Skillet
	{
		PrimaryAttack();
	} 
	else if ( (pOwner->m_nButtons & IN_ATTACK2) && (m_flNextSecondaryAttack <= gpGlobals->curtime)
		&& ( m_iFireInputType == FIREINPUTTYPE_CANHOLD || pOwner->m_afButtonPressed & IN_ATTACK2 ) ) //DHL - Skillet
	{
		SecondaryAttack();
	}
	else 
