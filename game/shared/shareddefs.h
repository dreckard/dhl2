...
#define ITEM_FLAG_NODROP			(1<<8)  // DHL: This weapon may not be dropped (Kung fu, etc.)
...
#define DMG_BLEED			(1 << 22)   // DHL, bleeding. Dont forget to change the other << x 's on removal
...
struct FireBulletsInfo_t
{
	FireBulletsInfo_t()
	{
		...
		//DHL - Skillet
		m_flDamageScalar = 1.0f;
		m_flTotalDistancePenetrated = 0.0f;
		m_iTimesPenetrated = 0;
	...
	//DHL - Skillet
	float m_flDamageScalar;
	float m_flTotalDistancePenetrated;
...
struct EmitSound_t
{
	EmitSound_t() :
		...
		m_bLooping( false ), //DHL - Skillet
		m_bFromSoundPatch( false ), //DHL - Skillet
		m_nSpeakerEntity( -1 ),
		m_UtlVecSoundOrigin(),
	...
	bool						m_bLooping; //DHL - Skillet - Loop?
	bool						m_bFromSoundPatch; //DHL - Skillet - Is this being emitted by a sound patch?
