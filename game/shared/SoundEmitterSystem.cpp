...
//DHL
bool bUseEmitSndPitch = false;
#ifdef CLIENT_DLL
	//CUtlVector<CSoundPatch*> DHLShared::m_SndList;
#endif

#ifdef USE_FMOD
void __MsgFunc_EmitSound( bf_read &msg );
void __MsgFunc_StopSound( bf_read &msg );

static ConVar dhl_fmod_bypass( "dhl_fmod_bypass", "0", FCVAR_REPLICATED | FCVAR_CHEAT, "Force sounds to bypass FMOD and use default HL2 sound system" );
#ifdef CLIENT_DLL
unsigned int length = 0;
void *buffer = NULL;
FMOD::DSP* locationalUnderwaterDSP = NULL;

static ConVar dhl_fmod_software( "dhl_fmod_software", "0", FCVAR_CHEAT, "Use software for all FMOD sounds" );
static ConVar dhl_fmod_visualize( "dhl_fmod_visualize", "0", FCVAR_CHEAT, "Show locations of FMOD sounds" );
static ConVar dhl_fmod_nomemstream( "dhl_fmod_nomemstream", "0", FCVAR_CHEAT, "Disable memory (GCF) streaming for sounds" );

#ifdef _DEBUG
//DHL - Skillet - Shutdown FMOD
void CC_FMOD_Shutdown( void )
{
	DHLShared::GetFMODSystem()->release();
	DHLShared::SetFMODSystem( NULL );
}
static ConCommand dhl_fmod_shutdown( "dhl_fmod_shutdown", CC_FMOD_Shutdown, "Shutdown FMOD and release all assets (Debug only)", FCVAR_CHEAT );
#endif //_DEBUG
#endif //CLIENT_DLL
#endif //USE_FMOD
...
#ifdef CLIENT_DLL
#ifdef USE_FMOD
		usermessages->HookMessage( "EmitSound", __MsgFunc_EmitSound );
		usermessages->HookMessage( "StopSound", __MsgFunc_StopSound );
#endif
#endif
...
//---------------------------------------------------------------------------------------------------
//DHL - Skillet - FMOD version of EmitSound, if it fails then we fall back to Source's sound system.
//---------------------------------------------------------------------------------------------------
#ifdef CLIENT_DLL
#ifdef USE_FMOD
	bool EmitSoundFMOD( const EmitSound_t &ep, bool bFromServer = false, bool bFromMemory = false, FMOD::Channel** inputChan = NULL )
	{
		//Can't play a sound if we don't know its name
		if ( !ep.m_pSoundName && !bFromMemory )
			return false;

		bool bSpatialized = false;
		bool bNoSlowing = false;
		bool bWeaponSound = false;
		bool bDirectPlay = false;

		ConVar* dhl_fmod_debug = cvar->FindVar( "dhl_fmod_debug" );
		FMOD::System* fmod_system = DHLShared::GetFMODSystem();
		if ( !fmod_system )
			return false;
		...
		char soundName[256];
		std::string str;
		std::string relativeSoundName;
		if ( !bFromMemory )
		{
			if ( ( Q_stristr( ep.m_pSoundName, ".wav" ) || 
			Q_stristr( ep.m_pSoundName, ".mp3" ) ||
			ep.m_pSoundName[0] == '!' ) )
			{
				//Try and look up the parameters
				str = ep.m_pSoundName;
				params.volume = ep.m_flVolume;
				params.soundlevel = ep.m_SoundLevel;
				bDirectPlay = true;
			}
			else
			{
				soundemitterbase->GetParametersForSound( ep.m_pSoundName, params, GENDER_NONE );
				str = params.soundname;

				//Not a valid sound name
				if ( Q_strlen( params.soundname ) < 3 )
					return false;
			}

			if ( DHLShared::IsSoundExcluded( PSkipSoundChars(str.c_str()) ) )
				return false;

			unsigned int charPos = 0;
			//Clean out formatting characters
			charPos = str.rfind( ")", 2 );
			if ( charPos != std::string::npos )
			{
				//This flag indicates that the sound should be 3D
				bSpatialized = true;
			}
			...
			//HUD stuff, don't slow it
			charPos = str.find( "common/", 0 );
			if ( charPos != std::string::npos )
				bNoSlowing = true;

			charPos = str.find( "suit_", 0 );
			if ( charPos != std::string::npos )
				bNoSlowing = true;

			//Let people (e.g. custom mappers) plop sounds in here that they don't want slowed
			charPos = str.find( "noslow/", 0 );
			if ( charPos != std::string::npos )
				bNoSlowing = true;

			//Decide which group this sound should go into
			charPos = str.find( "weapons/", 0 );
			if ( charPos != std::string::npos )
				bWeaponSound = true;

			char relName[128];
			Q_strcpy( soundName, str.c_str() );
			DHLShared::FormatSoundName( soundName, relName );
			str = soundName;

			if ( relName )
			{
				relativeSoundName = relName;
				/*charPos = relativeSoundName.rfind( ")", 2 );
				if ( charPos != std::string::npos )
					relativeSoundName.erase( charPos, 1 );
		...
		FMOD_MODE flags = 0;
		if ( dhl_fmod_software.GetBool() )
			flags |= FMOD_SOFTWARE;

		//Test, only allow each sound to have one concurrent instance
		FMODSound_t *pSoundSearch = DHLShared::LookupFMODSound( soundName );
		if ( pSoundSearch && pSoundSearch->pChannel )
		{
			if ( (pSoundSearch->iCreatorIndex == ep.m_nSpeakerEntity) && !bWeaponSound )
				pSoundSearch->pChannel->stop();
		}
		...
		if ( pLocalPlayer )
		{
			if ( (pLocalPlayer->entindex() == ep.m_nSpeakerEntity) && (pLocalPlayer->GetHealth() > 0) )
				bFromLocalPlayer = true;
		}

		//If we have an origin, and we're not being played by the local player, then it's a 3D sound, otherwise 2D
		if ( ep.m_pOrigin && !bFromLocalPlayer )
		{
			if ( bFromMemory )
			{
				...
				exinfo.length = length; //ep.m_nChannel;
				flags |= (FMOD_3D | FMOD_OPENMEMORY);
				result = fmod_system->createSound( (const char*)buffer, flags, &exinfo, &fmodSound );
			}
			else
			{
				flags |= FMOD_3D;
				result = fmod_system->createStream( soundName, flags, NULL, &fmodSound );
				if ( dhl_fmod_debug->GetBool() )
				{
					if ( bFromServer )
						Msg( "Playing server called sound: %s as 3D \n", soundName );
					else
						Msg( "Playing locally called sound: %s as 3D \n", soundName );

				}
			}

			//We scale our stuffs according to HL2's scripted "Soundlevel"
			float flSoundLvl = float(params.soundlevel);

			//This function is not what it first seems - read the FMOD documentation before messing with it
			fmodSound->set3DMinMaxDistance( FMOD_UNIT_FACTOR * (flSoundLvl / SNDLVL_NORM), FMOD_UNIT_FACTOR * 50000.0f );
		}
		else
		{
			if ( bFromMemory )
			{
				...
				//result = fmod_system->createStream( (const char*)buffer, flags, &exinfo, &fmodSound );
				result = fmod_system->createSound( (const char*)buffer, flags, &exinfo, &fmodSound );
			}
			else
			{
				result = fmod_system->createStream( soundName, flags, 0, &fmodSound );
				if ( dhl_fmod_debug->GetBool() )
				{
					if ( bFromServer )
						Msg( "Playing server called sound: %s as 2D \n", soundName );
					else
						Msg( "Playing locally called sound: %s as 2D \n", soundName );

				}
			}
		}
		//Had a fatal error, deal with it.
		if ( result != FMOD_OK )
		{
			//Try loading it through the Valve filesystem (access to GCFs and what not)
			if ( result == FMOD_ERR_FILE_NOTFOUND && !bFromMemory && !dhl_fmod_nomemstream.GetBool() )
			{
				relativeSoundName.insert( 0, "sound/" );
				DHLShared::LoadFileIntoMemory( relativeSoundName.c_str(), &buffer, &length );
				if ( buffer )
				{
					EmitSound_t newEp;
					newEp.m_flVolume = params.volume;
					newEp.m_SoundLevel = params.soundlevel;
					newEp.m_pOrigin = ep.m_pOrigin;
					newEp.m_nSpeakerEntity = ep.m_nSpeakerEntity;
					//newEp.m_pSoundName = (const char *)buffer;
					newEp.m_pSoundName = NULL;
					newEp.m_bLooping = ep.m_bLooping;
					//newEp.m_nChannel = length; //HACK! We store the length here when playing from memory

					FMOD::Channel *memstreamchan;
					bool bReturn = EmitSoundFMOD( newEp, bFromServer, true, &memstreamchan );

					//Free that memory or we'll end up with a mega leak
					free( buffer );
					buffer = NULL;
					...
					if ( bReturn )
					{
						...
						Q_strcpy( soundStruct->name, soundName );
						soundStruct->pChannel = memstreamchan;
						soundStruct->iCreatorIndex = ep.m_nSpeakerEntity;
						//soundStruct->buffer = buffer;
						DHLShared::GetPlayingChannelsList()->AddToTail( soundStruct );
					}
					return bReturn;
				}
			}
		...
		{
			if ( flags & FMOD_3D && dhl_fmod_visualize.GetBool() )
			{
				char Command[128];
				Q_snprintf( Command, sizeof( Command ), "drawcross %f %f %f \n", ep.m_pOrigin->x, ep.m_pOrigin->y, ep.m_pOrigin->z );
		...
		if ( ep.m_bLooping )
			...
			fmodSound->setMode( FMOD_LOOP_OFF );

		result = fmod_system->playSound( FMOD_CHANNEL_FREE, fmodSound, true, &channel );
		//Had a fatal error, use Source's sound system
		if ( DHLShared::HandleFMODError( result ) )
				return false;

		if ( channel )
		{
			//Time to sort them into channel groups
			FMODChannelGroups* channelGroups = DHLShared::GetChannelGroups();
			if ( bNoSlowing )
				channel->setChannelGroup( channelGroups->pChanNoSlow );
			else
			{
				if ( bWeaponSound )
					channel->setChannelGroup( channelGroups->pChanWeapons );
				else
					channel->setChannelGroup( channelGroups->pChanOther );

				//Scale frequency
				channel->setFrequency( DHLShared::GetScaledFrequency( channel, DHLRules()->GetTimescale() ) );
			}
		...
		//Handle 3D stuff if this is a 3D sound
		if ( flags & FMOD_3D )
		{
			if ( !locationalUnderwaterDSP )
			{
				//Underwater DSP effect
				DHLShared::GetFMODSystem()->createDSPByType( FMOD_DSP_TYPE_LOWPASS, &locationalUnderwaterDSP );
				locationalUnderwaterDSP->setParameter( 0, 3000.0f ); //Frequency Cutoff
				locationalUnderwaterDSP->setParameter( 1, 1.02f ); //Resonance
			}
			FMOD_VECTOR fmodOrigin;
			FMOD_VECTOR fmodVel = { 0, 0, 0 };
			DHLShared::VectorToFMOD( Vector( ep.m_pOrigin->x, ep.m_pOrigin->y, ep.m_pOrigin->z), &fmodOrigin );
			channel->set3DAttributes( &fmodOrigin, &fmodVel );
			if ( enginetrace->GetPointContents( Vector( ep.m_pOrigin->x, ep.m_pOrigin->y, ep.m_pOrigin->z)) & MASK_WATER )
				channel->addDSP( locationalUnderwaterDSP );

		}

		channel->setCallback( FMOD_CHANNEL_CALLBACKTYPE_END, DHLShared::endCallback, 0 );
		if ( !bFromMemory )
		{
			FMODSound_t* soundStruct = new FMODSound_t;
			Q_strcpy( soundStruct->name, soundName );
			soundStruct->pChannel = channel;
			soundStruct->iCreatorIndex = ep.m_nSpeakerEntity;
			DHLShared::GetPlayingChannelsList()->AddToTail( soundStruct );
		}
		if ( inputChan )
			*inputChan = channel;
		//After we've set everything up play the sound.
	...
	{
		const char *editedName = ep.m_pSoundName;

		int iPitch = ep.m_nPitch; //DHL - Skillet
		#ifdef USE_FMOD
			if ( !dhl_fmod_bypass.GetBool() )
			{
				editedName = PSkipSoundChars( ep.m_pSoundName );
				//DHL - Skillet - Every single sound comes through here just before being played, so we'll hijack it with FMOD
				#ifdef CLIENT_DLL
						//Try to play it with FMOD, if successful then we're done, otherwise play it through Source
						if ( EmitSoundFMOD( ep ) )
							return;
				#else
					if ( !(DHLShared::IsSoundExcluded( editedName )) )
					{
							Vector vecOrigin;
							if ( ep.m_pOrigin )
								vecOrigin = *ep.m_pOrigin;
							//Most sounds are like this - no origin is specified, so use that of the emitting entity
							else if ( ep.m_nSpeakerEntity > 0 )
								vecOrigin = UTIL_EntityByIndex( ep.m_nSpeakerEntity )->GetAbsOrigin();

							CRecipientFilter msgFilter;
							msgFilter.CopyFrom( (CRecipientFilter &)filter );

							//Send it over the wire
							UserMessageBegin( msgFilter, "EmitSound" );
								WRITE_STRING( ep.m_pSoundName );
								WRITE_VEC3COORD( vecOrigin );
								WRITE_BYTE( ep.m_SoundLevel );
								WRITE_SHORT( ep.m_nSpeakerEntity );
								WRITE_FLOAT( ep.m_flVolume );
								WRITE_BOOL( ep.m_bLooping );
							MessageEnd();
							return;
					}
				#endif
		...
		#else //!USE_FMOD
			#ifdef CLIENT_DLL
				/*if ( !ep.m_bFromSoundPatch )
				{
					const char* wav = soundemitterbase->GetWavFileForSound( ep.m_pSoundName, GENDER_NONE );
					CSoundPatch* pPatch = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex, ep.m_nChannel, wav, ep.m_SoundLevel );
					if ( pPatch )
					{
						CSoundEnvelopeController::GetController().Play( pPatch, ep.m_flVolume, (float)(ep.m_nPitch) );
						CSoundEnvelopeController::GetController().CommandAdd( pPatch, enginesound->GetSoundDuration( wav ) * 4, SOUNDCTRL_DESTROY, 0.0f, 0.0f );
						//Msg( "Duration: %f \n", enginesound->GetSoundDuration( wav ) );
						return;
					}
				}*/
			#endif
			iPitch *= DHLRules()->GetTimescale();
		#endif
	
		VPROF( "CSoundEmitterSystem::EmitSound (calls engine)" );
		if ( editedName && 
			( Q_stristr( editedName, ".wav" ) || 
			  Q_stristr( editedName, ".mp3" ) || 
			  editedName[0] == '!' ) )
		{
#if !defined( CLIENT_DLL )
			bool bSwallowed = CEnvMicrophone::OnSoundPlayed( 
			...
			if ( ep.m_bWarnOnDirectWaveReference && 
				Q_stristr( editedName, ".wav" ) )
			{
				WaveTrace( editedName, "Emitsound" );
			}

#if defined( _DEBUG ) && !defined( CLIENT_DLL )
			if ( !enginesound->IsSoundPrecached( editedName ) )
			{
			...
			if ( ep.m_pflSoundDuration )
			{
				*ep.m_pflSoundDuration = enginesound->GetSoundDuration( editedName );
			}
			...
			#ifdef CLIENT_DLL
				//DHL - Skillet
				/*if ( !(ep.m_nFlags & (SND_STOP | SND_CHANGE_PITCH | SND_CHANGE_VOL)) )
				{
					SourceSound_t nSourceSnd( enginesound->GetGuidForLastSoundEmitted(), NULL, ep );
					DHLShared::m_SndList.AddToTail( nSourceSnd );
				}*/
			#endif
		...
		{
			ep.m_hSoundScriptHandle = (HSOUNDSCRIPTHANDLE)soundemitterbase->GetSoundIndex( editedName );
		}
		...
		//DHL - Skillet
		if ( iPitch != ep.m_nPitch && !(ep.m_nFlags & SND_CHANGE_PITCH) )
		{
			EmitSound_t& newEp = const_cast<EmitSound_t&>(ep);
			newEp.m_nPitch = iPitch;
			bUseEmitSndPitch = true;
			const_cast<EmitSound_t&>(ep) = newEp;
			//EmitSoundByHandle( filter, entindex, newEp, ep.m_hSoundScriptHandle );
			//return;
		}
	...
	#ifdef CLIENT_DLL
	#ifdef USE_FMOD
	//DHL - Skillet - Stop an FMOD sound
	void StopSoundFMOD( int entindex, const char *soundname )
	{
		int soundindex = soundemitterbase->GetSoundIndex( soundname );
		if ( soundindex == -1 )
		{
		...
		for ( int i = 0; i < c; ++i )
		{
			...
			char name[256];
			Q_strncpy( name, wavename, 256 );
			DHLShared::FormatSoundName( name );
			FMODSound_t* pSound = DHLShared::LookupFMODSound( name );
			while ( pSound && pSound->pChannel )
			{
				//Entities can only stop sounds that they created
				//If the index is less than zero then one wasn't specified, stop all sounds regardless of the owner
				if ( pSound->iCreatorIndex == entindex || entindex < 0 )
				{
					pSound->pChannel->stop();
					DHLShared::SoundEnded( pSound->pChannel );
				}
				pSound = DHLShared::LookupFMODSound( wavename );
			}
		}
	}
	#endif
...
#ifdef USE_FMOD
#ifdef CLIENT_DLL
		StopSoundFMOD( entindex, soundname );
#else
		//Network the StopSound message
		//Send it to everyone
        CBroadcastRecipientFilter msgFilter;

		UserMessageBegin( msgFilter, "StopSound" );
	...
	void EmitAmbientSound( int entindex, const Vector &origin, const char *pSample, float volume, soundlevel_t soundlevel, int flags, int pitch, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
	{
		...
		ep.m_nPitch = pitch;
#else
		//pitch *= DHLRules()->GetTimescale(); //DHL - Skillet - Don't change pitch for ambient sounds yet
#endif
...
//DHL - Usermessage stuff
#ifdef CLIENT_DLL
#ifdef USE_FMOD
//DHL - Skillet - Client message handler
	void __MsgFunc_EmitSound( bf_read &msg )
	{
		char szMsgSoundName[128];
		msg.ReadString( szMsgSoundName, 128, false );
		Vector vecMsgOrigin;
		msg.ReadBitVec3Coord( vecMsgOrigin );
		int iMsgSoundLevel = msg.ReadByte();
		int iMsgSpeakerEnt = msg.ReadShort();
		int iMsgVolume = msg.ReadFloat();
		bool bLooping;
		msg.ReadBits( &bLooping, 1 );

		EmitSound_t msgEp;
		msgEp.m_pSoundName = szMsgSoundName;
		msgEp.m_pOrigin = &vecMsgOrigin;
		msgEp.m_SoundLevel = soundlevel_t(iMsgSoundLevel);
		msgEp.m_nSpeakerEntity = iMsgSpeakerEnt;
		msgEp.m_flVolume = iMsgVolume;
		msgEp.m_bLooping = bLooping;
		g_SoundEmitterSystem.EmitSoundFMOD( msgEp, true );
	}
	...
	void __MsgFunc_StopSound( bf_read &msg )
	{
		...
		g_SoundEmitterSystem.StopSoundFMOD( index, szMsgSoundName );
	}
#endif
#endif
...
void CBaseEntity::EmitSound( const char *soundname, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
{
	...
	params.m_nSpeakerEntity = entindex(); //DHL - Skillet
...
void CBaseEntity::EmitSound( IRecipientFilter& filter, int iEntIndex, const char *soundname, const Vector *pOrigin /*= NULL*/, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
{
	...
	//DHL - Skillet - Save this entity's index
...
void CBaseEntity::EmitSound( IRecipientFilter& filter, int iEntIndex, const EmitSound_t & params )
{
	...
	//DHL - Skillet
	/*#ifdef CLIENT_DLL
		if ( !params.m_pOrigin && (params.m_nSpeakerEntity != C_BasePlayer::GetLocalPlayer()->entindex()) )
...
void CBaseEntity::EmitSound( IRecipientFilter& filter, int iEntIndex, const EmitSound_t & params, HSOUNDSCRIPTHANDLE& handle )
{
	...
	//DHL - Skillet - HACKHACK - Emit by handle dosen't play nice with FMOD, so don't use it
