...
//DHL
//Seconds after landing from a stunt before being allowed to stand up
#define DHL_STUNT_RECOVERY_TIME 0.6
...
{
	//DHL - Skillet - Compute collision bbox position based on animation (how expensive is this?)
	if ( !ducked && DHLPlayer->m_bProne )
	{
		return VEC_PRONE_HULL_MIN;
		//DHLPlayer->ComputeEntitySpaceHitboxSurroundingBox( &mins, &maxs );
		/*Vector origin = vec3_origin;
		Vector localOrigin = vec3_origin;
		QAngle angles = vec3_angle;
		DHLPlayer->GetBonePosition( 0, origin, angles );
		DHLPlayer->WorldToEntitySpace( origin, &localOrigin );
		mins = localOrigin + VEC_PRONE_HULL_MIN;
		mins.z = 0;
		return mins;*/
	}
...
void CGameMovement::ProcessMovement( CBasePlayer *pPlayer, CMoveData *pMove )
{
	...
	//DHL - Scale down frametime (and movement speed)
	DHLShared::ScaleFrametime();
...
void CGameMovement::Accelerate( Vector& wishdir, float wishspeed, float accel )
{
	...
	if ( !CanAccelerate() )
		return;
	//---------------------------------------------------------------------------------------
	//DHL wallstrafe speed fix - Credit to Tony "omega" Sergi
	//http://developer.valvesoftware.com/wiki/SDK_Known_Issues_List#Wall_Bosting_Exploit
	//---------------------------------------------------------------------------------------
...
void CGameMovement::FullWalkMove( )
{
	...
	// Not fully underwater
	{
		...
		//DHL - Skillet - Check for the stunt key to be pressed
		if (mv->m_nButtons & IN_STUNT)
		{
			Stunt();
		}
		else
		{
			mv->m_nOldButtons &= ~IN_STUNT;
		}
...
void CGameMovement::FullNoClipMove( float factor, float maxacceleration )
{
	...
	if ( !sv_cheats.GetBool() && !player->IsObserver() && player->GetMoveType() == MOVETYPE_NOCLIP )
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	if ( DHLPlayer->m_bProne ) //DHL - Skillet - Stand up if we're prone, then stop the jump
	{
		Prone( false );
		return false;
	}
	if ( DHLPlayer->GetStuntState() == STUNT_JUMPED && !(mv->m_nOldButtons & IN_JUMP) ) //DHL - Skillet - Try to climb
	{
		TryClimb();
		return false;
	}
	...
	{
		//DHL - Skillet - multiply jump height by 1.5
		mv->m_vecVelocity[2] += flGroundFactor * flMul * 1.5;  // 2 * gravity * height
	}
	...
	//DHL - Skillet
	DHLPlayer->SetStuntState( STUNT_JUMPED );
...
//-----------------------------------------------------------------------------
// Purpose: DHL Stunt Code
//-----------------------------------------------------------------------------
bool CGameMovement::Stunt( void )
{
	if ( mv->m_nOldButtons & IN_STUNT ) //No holding key down.
		return false;
	if ( DHLPlayer->GetFlags() & FL_DUCKING ) //Not if we're ducking
		return false;
	if ( DHLPlayer->GetWaterLevel() >= 2 ) //Or chest deep in water
		return false;
 	//if ( DHLPlayer->GetGroundEntity() == NULL ) // When in Valveland, do as the Valvelandians.
		//return false;
	...
	//Don't even bother unless they're moving faster than this.
	//50.0 is about 25% of normal HL2 movement speed.
	if ( (abs( mv->m_vecVelocity.x ) + abs( mv->m_vecVelocity.y )) < 50.0f )
		return false;

	//NEW 6/4/07: Jump-diving
	//Player can still dive if in the air and not moving downwards (at or prior to apex)
	if ( !DHLPlayer->GetGroundEntity() )
	{
		if ( DHLPlayer->GetStuntState() == STUNT_JUMPED )
		{
			//Vector vecMoveDir = mv->m_vecVelocity;
			//VectorNormalize( vecMoveDir );
			if ( mv->m_vecVelocity.z < -75.0f || DHLPlayer->GetWaterLevel() > 0 )
				return false;

			//Look for a wall to dive off of
			Vector vecForward = vec3_origin;
			//DHLPlayer->EyeVectors( &vecForward );
			AngleVectors( player->GetAbsAngles(), &vecForward );
			//Vector vecMaxs = GetPlayerMaxs(false); Vector vecMins = GetPlayerMins(false);
			//vecMaxs *= 0.75; vecMins *= 0.75;
			trace_t tr;
			//UTIL_TraceHull( mv->GetAbsOrigin(), mv->GetAbsOrigin() + vecForward * 32.0f, vecMins, vecMaxs, MASK_SOLID_BRUSHONLY,
			//	DHLPlayer, COLLISION_GROUP_NONE, &tr );
			Vector vecStartPos = mv->GetAbsOrigin();
			vecStartPos.z += GetPlayerMaxs(false).z / 2;
			UTIL_TraceLine( vecStartPos, vecStartPos + vecForward * 48.0f, MASK_SOLID_BRUSHONLY, DHLPlayer, COLLISION_GROUP_NONE, &tr );
			if ( tr.DidHit() )
			{
				Vector vecNewDir = (tr.endpos - mv->GetAbsOrigin()) - 2 * tr.plane.normal * tr.plane.normal.Dot( (tr.endpos - mv->GetAbsOrigin()) );
				VectorNormalize( vecNewDir );

				mv->m_vecVelocity = vecNewDir * 400.0f;
				mv->m_vecVelocity.z *= 0.75f;
				DHLPlayer->SetStuntState( STUNT_DIVE );
				Prone( true ); //Go Prone
				mv->m_nOldButtons |= IN_STUNT; //Reset the stunt key
				DHLPlayer->CollisionProp()->SetSurroundingBoundsType( USE_SPECIFIED_BOUNDS, &VEC_STUNT_TR_MIN, &VEC_STUNT_TR_MAX );
				return true;
			}
	...
	Vector vecStuntDir;
	vecStuntDir = mv->m_vecVelocity;
	vecStuntDir.z = 0; //Disregard the player's movement pitch
	VectorNormalize( vecStuntDir );

	QAngle tempAngle;
	VectorAngles( vecStuntDir, tempAngle );
	DHLPlayer->m_flLastDiveYaw = tempAngle[YAW];

	for ( int iAxis = 0; iAxis < 2; ++iAxis ) //Mul x and y vel
		vecStuntDir[ iAxis ] *= ( 325.0f );
	mv->m_vecVelocity = vecStuntDir;
	mv->m_vecVelocity.z = 300.0f; //Dive height
	SetGroundEntity(NULL); //Release the player from the ground.
	DHLPlayer->SetStuntState( STUNT_DIVE ); //Notify player
	//MoveHelper()->PlayerSetAnimation( PLAYER_DIVE ); //Set animation
	//DHLPlayer->DoAnimationEvent( PLAYERANIMEVENT_CUSTOM, ACT_DHL_DIVE_GENERIC );

	Prone( true ); //Go Prone
	mv->m_nOldButtons |= IN_STUNT; //Reset the stunt key, so the code dosen't loop while key is held down

	DHLPlayer->CollisionProp()->SetSurroundingBoundsType( USE_SPECIFIED_BOUNDS, &VEC_STUNT_TR_MIN, &VEC_STUNT_TR_MAX ); //Set the surrounding bounds appropriately
	return true;
}

//-----------------------------------------------------------------------------
// Purpose: DHL: Climbing
//-----------------------------------------------------------------------------
void CGameMovement::TryClimb( void )
{
	if ( player->GetGroundEntity() )
		return;
	...
	if ( player->GetWaterLevel() >= 2 )
		return;
	if ( mv->m_vecVelocity.z < -200.0f )
		return;

	trace_t tr;
	Vector vecStartPos = player->GetAbsOrigin();
	vecStartPos.z += GetPlayerMaxs(false).z / 2;
	Vector vecForward = vec3_origin;
	AngleVectors( player->GetAbsAngles(), &vecForward );
	UTIL_TraceLine( vecStartPos, vecStartPos + vecForward * 35.0f, MASK_SOLID_BRUSHONLY, player, COLLISION_GROUP_NONE, &tr );

	if ( tr.DidHit() )
	{
		DHLPlayer->SetStuntState( STUNT_CLIMBED );
		mv->m_vecVelocity.z = 200.0f;
	}
}

//-----------------------------------------------------------------------------
// Purpose: DHL: Prone - what happens after you dive.
//-----------------------------------------------------------------------------
void CGameMovement::Prone( bool bDown )
{
	if ( IsDead() )
		return;
...
void CGameMovement::ProneDown( void )
{
	DHLPlayer->m_bProne = true;
	DHLPlayer->AddFlag( FL_PRONE );

	DHLPlayer->SetViewOffset( VEC_PRONE_VIEW );
	DHLPlayer->m_bProneStandReady = false;
	...
	//Reset the timer
	DHLPlayer->m_flProneStandTime = 0.0f;
} 
void CGameMovement::ProneUp( void )
{
	if ( !CanUnProne() )
		return;

	if ( DHLPlayer->m_bProneStandReady == true )
	{
		DHLPlayer->m_bProne = false;
		DHLPlayer->RemoveFlag( FL_PRONE );
		DHLPlayer->SetStuntState( STUNT_NONE );

		DHLPlayer->SetViewOffset( VEC_VIEW );

		#ifndef CLIENT_DLL
			DHLPlayer->UnProne();
		#endif
		FixPlayerProneStuck();
		DHLPlayer->CollisionProp()->SetSurroundingBoundsType( USE_SPECIFIED_BOUNDS, &VEC_HULL_MIN, &VEC_HULL_MAX );
	}
}

//DHL
//-----------------------------------------------------------------------------
// Called every PlayerMove()
//-----------------------------------------------------------------------------
ConVar dhl_backspeed( "dhl_backspeed", "0.7", FCVAR_CHEAT | FCVAR_REPLICATED );
void CGameMovement::DHLThink( void )
{
	if ( DHLPlayer->m_bProne )
		ProneThink();

	if ( DHLPlayer->GetStuntState() == STUNT_JUMPED && DHLPlayer->GetGroundEntity() )
		DHLPlayer->SetStuntState( STUNT_NONE );

	if ( mv->m_flForwardMove < 0.0f )
		mv->m_flForwardMove *= dhl_backspeed.GetFloat();
}

//DHL
//-----------------------------------------------------------------------------
// Called every PlayerMove() while the player is prone
//-----------------------------------------------------------------------------
void CGameMovement::ProneThink( void )
{
	//This timer can get out of whack after map changes, so clamp it
	if ( DHLPlayer->m_flProneStandTime > gpGlobals->curtime + DHL_STUNT_RECOVERY_TIME )
		DHLPlayer->m_flProneStandTime = gpGlobals->curtime + DHL_STUNT_RECOVERY_TIME;

	if ( player->GetGroundEntity() != NULL && DHLPlayer->m_flProneStandTime == 0.0f )
	{
		//We've landed
		DHLPlayer->m_flProneStandTime = gpGlobals->curtime + DHL_STUNT_RECOVERY_TIME;
		DHLPlayer->SetStuntState( STUNT_PRONE );
		DHLPlayer->CollisionProp()->SetSurroundingBoundsType( USE_SPECIFIED_BOUNDS, &VEC_PRONE_TR_MIN, &VEC_PRONE_TR_MAX );

		//Determine which direction they're facing by taking the difference in angles between when they dove and when they landed (now)
		float angDeg = DHLPlayer->m_flLastDiveYaw - DHLPlayer->GetLocalAngles()[YAW];
		if ( angDeg < 0.0f ) angDeg += 360.0f; //Make everything positive
		if ( angDeg > 360.0f ) angDeg -= 360.0f; //Between 0 and 360
		if ( angDeg <= 45.0f || angDeg >= 315.0f )
			DHLPlayer->m_iStuntDir = STUNTDIR_FORWARDS;
		else if ( angDeg >= 135.0f && angDeg <= 225.0f )
			DHLPlayer->m_iStuntDir = STUNTDIR_BACKWARDS;
		else if ( angDeg < 135.0f )
			DHLPlayer->m_iStuntDir = STUNTDIR_LEFT;
		else
			DHLPlayer->m_iStuntDir = STUNTDIR_RIGHT;
	}

	//Ok, the purpose of this only being run on the server side is so that the client does not erroneously predict that the
	//waiting period after the player lands to stand up has expired, causing some pretty ugly errors with the view offset
	//-Skillet
#ifndef CLIENT_DLL
	if ( !DHLPlayer->m_bProneStandReady && DHLPlayer->m_flProneStandTime <= gpGlobals->curtime && DHLPlayer->m_flProneStandTime != 0.0f )
		DHLPlayer->m_bProneStandReady = true;
#endif

	//Keep this from getting out of whack
	if ( DHLPlayer->GetViewOffset() != VEC_PRONE_VIEW )
		DHLPlayer->SetViewOffset( VEC_PRONE_VIEW );

	//Stand up if we're prone and underwater
	if ( DHLPlayer->GetWaterLevel() == 3 )
	{
		DHLPlayer->m_bProneStandReady = true;
		Prone( false );
	}

	if ( DHLPlayer->GetGroundEntity() != NULL )
	{
		HandleProneSpeedCrop();

		//Switch the prone animation to forwards for rolling if they're moving at a moderate speed in the x/y
		if ( abs(mv->m_flSideMove) >= DHL_WALK_SPEED * DHL_PRONESPEED_SCALAR ) //Use walk speed to allow some leeway
			DHLPlayer->m_iStuntDir = STUNTDIR_FORWARDS;
	}
}

void CGameMovement::HandleProneSpeedCrop( void )
{
	if ( !DHLPlayer->m_bProne )
		return;
	//DHL - Skillet - Scale down movement speed when prone - Intended to only move side to side at the moment (TS-style)
	float frac = 0.0f;
	//if ( DHLPlayer->m_iStuntDir == STUNTDIR_FORWARDS ) //Only allow movement from forward dive
		frac = DHL_PRONESPEED_SCALAR;
	mv->m_flForwardMove	*= 0.0f;
	mv->m_flSideMove	*= frac;
	mv->m_flUpMove		*= 0.0f;
	m_bProneSpeedCropped		= true;
}

//DHL: Copy of FixPlayerCrouchStuck
void CGameMovement::FixPlayerProneStuck( void )
{
	...
	for ( i = 0; i < ( VEC_HULL_MAX.z - VEC_PRONE_HULL_MAX.z ); i++ )
	{
		//mv->GetAbsOrigin().z += direction;
		Vector tempvec = mv->GetAbsOrigin();
		tempvec.z += direction;
		mv->SetAbsOrigin( tempvec );
		hitent = TestPlayerPosition( mv->GetAbsOrigin(), COLLISION_GROUP_PLAYER_MOVEMENT, dummy );
		if (hitent == INVALID_ENTITY_HANDLE )
			return;
	}
	...
	//VectorCopy( test, mv->GetAbsOrigin() ); // Failed
}

//DHL: Copy of CanUnduck
bool CGameMovement::CanUnProne( void )
{
	int i;
	trace_t trace;
	Vector newOrigin;
...
int CGameMovement::TryPlayerMove( Vector *pFirstDest, trace_t *pFirstTrace )
{
	...
	for (bumpcount=0 ; bumpcount < numbumps; bumpcount++)
	{
		...
		{
			if ( !player->ShouldTouch( pm.m_pEnt, pm ) || !pm.m_pEnt->ShouldTouch( player, pm ) )
			{
				//Retrace
				CTraceFilterSkipTwoEntities filter( player, pm.m_pEnt, COLLISION_GROUP_PLAYER_MOVEMENT );
				UTIL_TraceHull( mv->GetAbsOrigin(), end, GetPlayerMins(), GetPlayerMaxs(), MASK_SOLID, &filter, &pm );
				mv->SetAbsOrigin( pm.endpos );
		 ...
		 player->m_Local.m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHOLD )
	{
		...
		else
		{
			...
			if ( player->m_Local.m_flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED )
			{
				...
				//DHL: Spawn some blood at their feet on fall
				//I can't swim Butch...
				//Are you crazy? The fall will kill you!
				trace_t tr;
				//Grab a straight down vector
				Vector vecDown( 0, 0, -1 );
				UTIL_TraceLine( player->GetLocalOrigin(), player->GetLocalOrigin() + (vecDown * 100.0f), MASK_SOLID, player, COLLISION_GROUP_NONE, &tr );
				if ( tr.DidHit() && player->m_Local.m_flFallVelocity > 650.0f )
				{
...
{
	//DHL
	if ( DHLPlayer->m_bProne || player->IsObserver() || IsDead() )
		return;
...
{
	if ( DHLPlayer->m_bProne || player->IsObserver() || IsDead() ) //DHL
		return;
...
// XBOX SERVER ONLY
	...
	else if ( !IsDead() && !player->IsObserver() && !player->IsInAVehicle() && !DHLPlayer->m_bProne ) //DHL - Skillet
	{
		if ( ( player->m_Local.m_flDuckJumpTime == 0.0f ) && ( fabs(player->GetViewOffset().z - GetPlayerViewOffset( false ).z) > 0.1 ) )
		{
			...
			//Assert(0); //DHL - Skillet - This happens all the time, I'm not sure why, but assert commented for now
...
void CGameMovement::PlayerMove( void )
{
	...
	//DHL: Deal with all of our prone stuff
	//Note - this MUST be called after at least CheckParameters for the movement cropping to work
	DHLThink();
