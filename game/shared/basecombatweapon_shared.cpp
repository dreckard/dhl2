...
void CBaseCombatWeapon::Precache( void )
{
	...
	// Msg( "Client got %s\n", GetClassname() );
#else
	//DHL - Skillet - Precache the bullet
	UTIL_PrecacheOther( "dhl_projectile" );
#endif
...
bool CBaseCombatWeapon::CanBeSelected( void )
{
	...
	return true; //HasAmmo(); //DHL - Since we can now drop weapons, allow empty ones to be selected
}
...
void CBaseCombatWeapon::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType )
{
	...
	//DHL - Skillet
	case TRACER_WHIZ_ONLY:
		#ifdef CLIENT_DLL
			FX_TracerSound( vNewSrc, tr.endpos, TRACER_TYPE_DEFAULT );
		#endif
		//UTIL_Tracer( vNewSrc, tr.endpos, iEntIndex, iAttachment, 0.0f, true, "WhizOnlyTracer" );
		break;
	}
}
...
void CBaseCombatWeapon::SetPickupTouch( void )
{
	...
	if ( gpGlobals->maxClients > 1 )
	{
		if ( GetSpawnFlags() & SF_NORESPAWN )
		{
			...
			SetNextThink( gpGlobals->curtime + 55.0f ); //DHL: After this amount of time being idle, the weapon is removed
		}
	}
...
//-----------------------------------------------------------------------------
#ifdef CLIENT_DLL
//DHL - Skillet - Per client autoreload var
ConVar cl_autoreload( "cl_autoreload", "1", FCVAR_USERINFO | FCVAR_ARCHIVE, "Determines whether weapons are automatically reloaded when empty.  Checked on respawn." );
#endif
bool CBaseCombatWeapon::ReloadOrSwitchWeapons( void )
{
	...
	else
	{
		...
		if ( UsesClipsForAmmo1() && 
			 ...
			 m_flNextSecondaryAttack < gpGlobals->curtime )
		{
			//DHL - Skillet - Only if our client var is enabled
			CDHL_Player *pDHLPlayer = ToDHLPlayer( pOwner );
			if ( pDHLPlayer )
			{
				if ( pDHLPlayer->m_bAutoReload )
				{
...
bool CBaseCombatWeapon::DefaultDeploy( char *szViewModel, char *szWeaponModel, int iActivity, char *szAnimExt )
{
	...
	//DHL - Skillet - Allow players to deploy weapons without ammo regardless of autoswitch setting
...
#endif
	SendWeaponAnim( iActivity );
	ToHL2MPPlayer(GetOwner())->DoAnimationEvent( PLAYERANIMEVENT_RELOAD ); //DHL - Skillet
	...
	if ( m_bReloadsSingly )
	{
		...
		if ((m_bInReload) && (m_flNextPrimaryAttack <= gpGlobals->curtime))
		{
			if ( pOwner->m_nButtons & (IN_ATTACK | IN_ATTACK2) && m_iClip1 > 0 )
			{
				...
				//DHL - Skillet - Argh, toggle the bits off so we can proceed without interfering with buttonpressed fire system
...
void CBaseCombatWeapon::PrimaryAttack( void )
{
	...
	ToHL2MPPlayer(pPlayer)->DoAnimationEvent( PLAYERANIMEVENT_ATTACK_PRIMARY ); //DHL - Skillet
